...
#prefix='lowHT_'
#qtVals=range(0,550,25)
#htVals=[0,50,100,150,200,250]
#
#c = ROOT.TChain("Events")
#c.Add("/data/schoef/convertedMetTuples2_v1//looseDoubleMu/DYJetsToLL/histo_DYJetsToLL_*")
...
#nvtx_rawMetPhi      = {'color':ROOT.kBlue, 'var':'rawMetPhi',     'niceName':'raw #phi(#slash{E}_{T})', 'histo': ROOT.TH1F('nvtx_rawMetPhi',         'nvtx_rawMetPhi',  len(nvtxVals)-1, array('d',nvtxVals) ), 'xTitle':'n-vtx'}
#nvtx_t1MetPhi       = {'color':ROOT.kGreen, 'var':'t1MetPhi',     'niceName':'T1 #phi(#slash{E}_{T})', 'histo': ROOT.TH1F('nvtx_t1MetPhi',          'nvtx_t1MetPhi', len(nvtxVals)-1, array('d',nvtxVals)   ),'xTitle':'n-vtx'}
#nvtx_t1TxyMetPhi    = {'color':ROOT.kRed, 'var':'t1TxyMetPhi',    'niceName':'T1Txy #phi(#slash{E}_{T})', 'histo': ROOT.TH1F('nvtx_t1TxyMetPhi',       'nvtx_t1TxyMetPhi',len(nvtxVals)-1, array('d',nvtxVals)  ),'xTitle':'n-vtx'}
#nvtx_txyMetPhi      = {'color':ROOT.kMagenta, 'var':'txyMetPhi',  'niceName':'Txy #phi(#slash{E}_{T})', 'histo': ROOT.TH1F('nvtx_txyMetPhi',         'nvtx_txyMetPhi', len(nvtxVals)-1, array('d',nvtxVals)  ),'xTitle':'n-vtx'}
#ht_phis = [\
#  ht_rawMetPhi, ht_t1MetPhi, ht_t1TxyMetPhi, ht_txyMetPhi
#  ]
#nvtx_phis = [\
#  nvtx_rawMetPhi, nvtx_t1MetPhi, nvtx_t1TxyMetPhi, nvtx_txyMetPhi
#  ]
#stuff=[]
#first=True
#
...
#    del phitmp
#for p in nvtx_phis:
#  for i_nvtx in range(len(nvtxVals)-1):
#    nvtx_b = nvtxVals[i_nvtx:i_nvtx+2]
##    print nvtx_b,p['var']+'>>'+p['histo'].GetName(), 'weight*(ht>'+str(nvtx_b[0])+'&&ht<'+str(nvtx_b[1])+')'
#    c.Draw(p['var']+'>>phitmp(50,-pi,pi)','weight*(ngoodVertices>='+str(nvtx_b[0])+'&&ngoodVertices<'+str(nvtx_b[1])+')','goff')
#    phitmp = ROOT.gDirectory.Get('phitmp')
#    cs=0
#    cc=0
#    c0=phitmp.Integral()/(2.*pi)
...
#    cc=cc/pi
##    print p['var'],c0,cs,cc,sqrt(cs**2+cc**2)/c0
#    p['histo'].Fill(0.5*sum(nvtx_b), sqrt(cs**2+cc**2)/c0)
#    del phitmp
#
#for p in ht_phis+nvtx_phis:
#  p['histo'].SetLineColor(p['color'])
#  p['histo'].SetMarkerColor(p['color'])
#  p['histo'].SetMarkerStyle(0)
#  p['histo'].GetXaxis().SetLabelSize(0.03)
...
#    p['histo'].Draw('hsame')
#
#l= ROOT.TLegend(0.55,0.95-0.06*len(ht_phis),.95,.95)
#for p in ht_phis:
#  l.AddEntry(p['histo'], p['niceName'])#+etab[0].split('_')[0]+", "+shortName[etab[0].split('_')[1]])
#
#l.SetFillColor(0)
...
#    p['histo'].Draw('hsame')
#
#l= ROOT.TLegend(0.10,0.95-0.06*len(nvtx_phis),.50,.95)
#for p in nvtx_phis:
#  l.AddEntry(p['histo'], p['niceName'])#+etab[0].split('_')[0]+", "+shortName[etab[0].split('_')[1]])
#
#l.SetFillColor(0)
...
#l.SetBorderSize(1)
#l.Draw()
#c1.Print('/afs/hephy.at/user/s/schoefbeck/www/pngMetPhiValidation/'+prefix+'fc_nvtx.png')
#c1.Print('/afs/hephy.at/user/s/schoefbeck/www/pngMetPhiValidation/'+prefix+'fc_nvtx.pdf')
#c1.Print('/afs/hephy.at/user/s/schoefbeck/www/pngMetPhiValidation/'+prefix+'fc_nvtx.root')
#del c1
#
##met scale
##zPt+ETmiss+u=0
...
#    scale=p['scale']['histo'].GetBinContent(p['scale']['histo'].FindBin(0.5*sum(qtb)))
#    if scale>0:
#      rmsOverScale=htmp.GetRMS()/scale
#      rmsEOverScale=htmp.GetRMSError()/scale
...
metPhiRes_rawMet      = {'color':ROOT.kBlue, 'var':'rawMet',     'niceName':'raw #slash{E}_{T}', 'histo': ROOT.TH1F('qt_rawMet',         'qt_rawMet',  len(qtVals)-1, array('d',qtVals)), 'xTitle':'Z q_{T} (GeV)'}
metPhiRes_t1Met       = {'color':ROOT.kGreen, 'var':'t1Met',     'niceName':'T1 #slash{E}_{T}', 'histo': ROOT.TH1F('qt_t1Met',          'qt_t1Met', len(qtVals)-1, array('d',qtVals)  ),'xTitle':'Z q_{T} (GeV)'}
metPhiRes_t1TxyMet    = {'color':ROOT.kRed, 'var':'t1TxyMet',    'niceName':'T1Txy #slash{E}_{T}', 'histo': ROOT.TH1F('qt_t1TxyMet',       'qt_t1TxyMet',len(qtVals)-1, array('d',qtVals) ),'xTitle':'Z q_{T} (GeV)'}
metPhiRes_txyMet      = {'color':ROOT.kMagenta, 'var':'txyMet',  'niceName':'Txy #slash{E}_{T}', 'histo': ROOT.TH1F('qt_txyMet',         'qt_txyMet', len(qtVals)-1, array('d',qtVals)),'xTitle':'Z q_{T} (GeV)'}
mets = [\
...
#  return 'acos(cos('+m+'Phi-phiZ))'
  return 'pi+atan2(-'+m+'Pt*sin('+m+'Phi)-ptZ*sin(phiZ), -'+m+'Pt*cos('+m+'Phi)-ptZ*cos(phiZ))-phiZ'

for p in mets:
  for i_qtv in range(len(qtVals)-1):
    htmp = ROOT.TH1F('htmp','htmp',100,-pi,pi)
    qtb = qtVals[i_qtv:i_qtv+2]
    ...
    p['histo'].SetBinError(p['histo'].FindBin(0.5*sum(qtb)),0)
...
for p in mets:
  ...
  p['histo'].GetYaxis().SetRangeUser(0,0.3)
  p['histo'].GetYaxis().SetLabelSize(0.03)
