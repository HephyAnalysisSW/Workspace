...
cPred.Add('/data/easilar/results2014/tauTuples/CSA14_TTJets_hadTauEstimate_cmg_large.root')
...
oneHadTau     = "(genTau_nMuNu+genTau_nMuE==0)&&genTau_nMuTau==1&&(Sum$(abs(LepGood_pdgId)==13)==1&&nLooseHardLeptons==1&&nTightHardLeptons==1)&&(Sum$(abs(LepGood_pdgId)==11)==0)&&nLepGood==1"
...
for htCut in [500]:
  for stCut in [250]:
    ...
    for nbtagsCut in [0,1,2]: 
      print 'ht:', htCut, 'st:',stCut ,'njet:',minNJetCut,maxNJetCut, 'nbtags > ' , nbtagsCut
      ...
      Path='/afs/hephy.at/user/e/easilar/www/hadronicTau_cmg/comparison_Results/ht_'+str(htCut)+'_st_'+str(stCut)+'_njet_'+str(minNJetCut)+'or'+str(maxNJetCut-1)+'_nbtag_=_'+str(nbtagsCut)+'/' ##'+str(nbtagsCut)+'/'
      #Path='/afs/hephy.at/user/e/easilar/www/hadronicTau_27Jan/ht_'+str(htCut)+'_st_'+str(stCut)+'_njet_'+str(minNJetCut)+'or'+str(maxNJetCut-1)+'_nbtag_'+str(nbtagsCut)+'/' ##'+str(nbtagsCut)+'/'
      ...
      DrawClosure(hMT,hMTPred,hMTPred110,hMTPred09,'MT',Path+'mT.pdf')
      DrawClosure(hMT,hMTPred,hMTPred110,hMTPred09,'MT',Path+'mT.root')
      ...
      DrawClosure(hHT,hHTPred,hHTPred110,hHTPred09,'HT',Path+'ht.pdf')
      DrawClosure(hHT,hHTPred,hHTPred110,hHTPred09,'HT',Path+'ht.root')
      ...
      DrawClosure(hST,hSTPred,hSTPred110,hSTPred09,'ST',Path+'ST.png')
      DrawClosure(hST,hSTPred,hSTPred110,hSTPred09,'ST',Path+'ST.pdf')
      DrawClosure(hST,hSTPred,hSTPred110,hSTPred09,'ST',Path+'ST.root')
      ...
      DrawClosure(hMET,hMETPred,hMETPred110,hMETPred09,'MET',Path+'met.pdf')
      DrawClosure(hMET,hMETPred,hMETPred110,hMETPred09,'MET',Path+'met.root')
      ...
      DrawClosure(hNJet,hNJetPred,hNJetPred110,hNJetPred09,'NJet',Path+'njets.pdf')
      DrawClosure(hNJet,hNJetPred,hNJetPred110,hNJetPred09,'NJet',Path+'njets.root')
      ...
      DrawClosure(hNBtag,hNBtagPred,hNBtagPred110,hNBtagPred09,'NBtag',Path+'nbtags.pdf')
      DrawClosure(hNBtag,hNBtagPred,hNBtagPred110,hNBtagPred09,'NBtag',Path+'nbtags.root')
  


      DrawClosure(hDeltaPhi,hDeltaPhiPred,hDeltaPhiPred110,hDeltaPhiPred09,'deltaPhi',Path+'deltaPhi.png')
      DrawClosure(hDeltaPhi,hDeltaPhiPred,hDeltaPhiPred110,hDeltaPhiPred09,'deltaPhi',Path+'deltaPhi.pdf')
      DrawClosure(hDeltaPhi,hDeltaPhiPred,hDeltaPhiPred110,hDeltaPhiPred09,'deltaPhi',Path+'deltaPhi.root')
      print 'DELTA PHI >1: '
      ePred = ROOT.Double()
      eTruth = ROOT.Double()
