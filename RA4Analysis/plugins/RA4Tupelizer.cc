#include "Workspace/RA4Analysis/plugins/RA4Tupelizer.h"
...
#include "Workspace/RA4Analysis/interface/ElectronEffectiveArea.h"
...
RA4Tupelizer::~RA4Tupelizer() {}

RA4Tupelizer::RA4Tupelizer( const edm::ParameterSet & pset):
  params_ ( pset ),
...
void RA4Tupelizer::beginJob ( )
{
  cout << "[RA4Tupelizer] starting ... " << endl;

  hlt_initialized_ = false;

}

void RA4Tupelizer::endJob()
{
  cout << endl;
  cout << "[RA4Tupelizer] shutting down ... " << endl;
}

void RA4Tupelizer::beginRun ( edm::Run & iRun, edm::EventSetup const & iSetup )
{
  bool changed(true);
...
int RA4Tupelizer::prescale(edm::Event & ev, const edm::EventSetup & setup, std::string hlt) {
//  return prod(hltConfig_.prescaleValues( ev, setup, hlt.c_str()));
  return hltConfig_.prescaleValue( ev, setup, hlt.c_str());
}


void RA4Tupelizer::produce( edm::Event & ev, const edm::EventSetup & setup) {
  ev_ = &ev;
  int peng(0);
...
//    cout << "Init HLT info" << endl;
    ...
    if (! this->hlt_initialized_ ) {
      ...
      RA4Tupelizer::hlt_initialized_ = true;
    }
  }
...
*/
  ...
  //  if (verbose_) cout << "[RA4Tupelizer] looking through " << genParticles.size() << " particles." << endl;
...
DEFINE_FWK_MODULE(RA4Tupelizer);
