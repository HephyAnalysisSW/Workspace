#ifndef Workspace_EarlyDataAnalysis_RA4Tupelizer_H
#define Workspace_EarlyDataAnalysis_RA4Tupelizer_H
...
#include "Workspace/RA4Analysis/interface/RecoHelper.h"
#include "Workspace/RA4Analysis/interface/MathHelper.h"
#include "Workspace/RA4Analysis/interface/ModelParameters.h"
...
class RA4Tupelizer : public edm::EDProducer
{
public:
...
//  }
  ...
  void put (const std::string & name, std::vector< U > & values) {
    if (types[name].find("[]")!=std::string::npos) {
      ...
      std::cout<<"[RA4Tupelizer] Warning! Var:" << name<<"  Type "<<types[name]<<" can't be filled with std::vector<> !"<<std::endl;
    }
  }
...
//    std::cout<<"Put typename.at(0):"<<name<<" Type: "<<types[name].at(0)<<"X"<<std::endl;
    try {
      switch (types[name].at(0)) {
        ...
        std::cout  << "[RA4Tupelizer] Error in put() for variable "<<name<<" (forgot the addVar?) with type "<<types[name]<< e.what() << std::endl;
    }
...
//  }
  ...
  explicit RA4Tupelizer ( const edm::ParameterSet & );
  ~RA4Tupelizer();
...
//    } catch (cms::Exception & e ) {
//      std::cout << "[RA4Tupelizer] error: " << e.what() << std::endl;
//    }
//    if (handle.isValid()) {
//      return &(*handle);
//    } else {
//      if (verbose) std::cout << "[RA4Tupelizer] " << " edm::Handle not valid for "<<name<<std::endl;
//      return NULL;
//    }
//  }
