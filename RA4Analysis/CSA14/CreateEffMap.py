...
h_AvarageEff = ROOT.TH1F('h_AvarageEff','h_AvarageEff',10,0,10)
h_AvarageEff.GetXaxis().SetBinLabel(1,'all')
h_AvarageEff.GetXaxis().SetBinLabel(2,'hasmatch')
h_AvarageEff.GetXaxis().SetBinLabel(3,'loose')
preselection = "ht>150 && njets>=3"
selection1="&&ngNuMuFromW==2&&ngNuEFromW==0"

c50 = ROOT.TChain('Events')
for b in ttJetsCSA1450ns['bins']:
  c50.Add(ttJetsCSA1450ns['dirname']+'/'+b+'/histo_ttJetsCSA1450ns_from*')
...
for relIso in [0.3]:
  for i in range(number_events):
    ...
    for p in range(int(ngLep)):
      ...
      if abs(gLepPdg) == 13 and gLepPt>15 and abs(gLepEta)<2.5:
        h_AvarageEff.Fill(1)
        print 'denominator gen muon Pt:', gLepPt,'eta:', gLepEta
        PtEtaDenDiLep.Fill(gLepPt,gLepEta)
        EtaDenDiLep.Fill(gLepEta)
        PtDenDiLep.Fill(gLepPt)
        if gLepInd>=0 and gLepDR<0.4:
          h_AvarageEff.Fill(2)
          print 'found a match! and genereted lepton index:',gLepInd , 'pt: ', gLepPt, 'eta:', gLepEta, 'phi:' , gLepPhi
          k=int(gLepInd)
          muon = getMu(c50,k)
          print 'Truth lepton pt:', muon['pt'], 'eta:', muon['eta'], 'phi:' , muon['phi']
          #muon = getLooseMuStage2(c50,k)
          muon['isTigt'] = tightPOGMuID(muon) 
          muon['isLoose'] = vetoMuID(muon,relIso)
          if muon['isLoose'] == 1 and abs(1-muon['pt']/gLepPt)<0.1:
            #if muon['isTight'] == 1 and abs(1-muon['pt']/gLepPt)<0.1:
            PtEtaNumDiLep.Fill(gLepPt,gLepEta)
            EtaNumDiLep.Fill(gLepEta)
            PtNumDiLep.Fill(gLepPt)
            h_AvarageEff.Fill(3)
  print 'average Eff:', h_AvarageEff.GetBinContent(4)/h_AvarageEff.GetBinContent(2) 
  ###Eff Calculation
