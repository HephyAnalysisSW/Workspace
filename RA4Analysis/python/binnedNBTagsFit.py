...
def binnedNBTagsFit(cut, MCcut, cutname, samples, configs, QCD_dict={}, bootstrap=False, res={}):
  ...
  templateDir       = configs['templateDir']
  nBTagVar          = configs['nBTagVar']
  templateWeights   = configs['templateWeights']
  templateWeightSuffix = configs['templateWeightSuffix']
  lumi              = configs['lumi']
  templateLumi      = configs['templateLumi']
  ...
  for n,nb in enumerate([(0,0),(1,1),(2,-1)]):
    hQCD.SetBinContent(n+1, QCD_dict[nb].central)
    hQCD.SetBinError(n+1, QCD_dict[nb].up)
...
#  frame_TTJets.Draw()
  ...
  c1.Print(printDir+'/'+cutname+'_nBTagFitRes.png')
  c1.Print(printDir+'/'+cutname+'_nBTagFitRes.pdf')
  c1.Print(printDir+'/'+cutname+'_nBTagFitRes.root')
  ...
  TT_AllPdg_frac  = asym_float(template_TTJets.GetBinContent(1), 0.,0.)
  TT_AllPdg_val   = asym_float(yield_TTJets.getVal(), yield_TTJets.getErrorHi(), abs(yield_TTJets.getErrorLo()))
  TT_AllPdg = 2 * TT_AllPdg_val * TT_AllPdg_frac
  
  W_PosPdg_frac  = asym_float(template_WJets_PosPdg.GetBinContent(1), 0.,0.)
  W_PosPdg_val   = asym_float(yield_WJets_PosPdg.getVal(), yield_WJets_PosPdg.getErrorHi(), abs(yield_WJets_PosPdg.getErrorLo()))
  W_PosPdg = W_PosPdg_val * W_PosPdg_frac

  W_NegPdg_frac  = asym_float(template_WJets_NegPdg.GetBinContent(1), 0.,0.)
  W_NegPdg_val   = asym_float(yield_WJets_NegPdg.getVal(), yield_WJets_NegPdg.getErrorHi(), abs(yield_WJets_NegPdg.getErrorLo()))
  W_NegPdg = W_NegPdg_val * W_NegPdg_frac

  Rest_PosPdg_frac  = asym_float(template_Rest_PosPdg.GetBinContent(1), 0.,0.)
  Rest_PosPdg_val   = asym_float(yield_Rest_PosPdg.getVal(), yield_Rest_PosPdg.getErrorHi(), abs(yield_Rest_PosPdg.getErrorLo()))
  Rest_PosPdg = Rest_PosPdg_val * Rest_PosPdg_frac

  Rest_NegPdg_frac  = asym_float(template_Rest_NegPdg.GetBinContent(1), 0.,0.)
  Rest_NegPdg_val   = asym_float(yield_Rest_NegPdg.getVal(), yield_Rest_NegPdg.getErrorHi(), abs(yield_Rest_NegPdg.getErrorLo()))
  Rest_NegPdg = Rest_NegPdg_val * Rest_NegPdg_frac
  res.update({
        'TT_AllPdg':TT_AllPdg,
        'W_PosPdg': W_PosPdg,
        'W_NegPdg': W_NegPdg,
        'Rest_PosPdg': Rest_PosPdg,
        'Rest_NegPdg': Rest_NegPdg,
        })
