...
#    self.additionalCutFunc=additionalCutFunc #Not yet implemented
    ...
    if self.binning.has_key('isExplicit') and self.binning['isExplicit']:
      self.histo = rClass(self.name, self.title,len(binning) - 1, array('d',binning))
    else:
...
#      if stk.options.has_key('yRange') and stk.options['yRange']:
      try:
...
#            else:
#              scaleFac = var.normalizeTo.histo.Integral()/hcopy.Integral()
#        else:
#          refValue = var.normalizeWhat.histo.Integral()
#          if refValue>0:
#            scaleFac = var.normalizeTo.histo.Integral()/refValue
#          else:
