...
path = os.path.abspath('../../HEPHYCommonTools/scripts/root')
if not path in sys.path:
    sys.path.insert(1, path)
del path
...
R.gROOT.ProcessLine(".L ../../HEPHYCommonTools/scripts/root/useNiceColorPalette.C")
...
parser.add_option("--plotObserved", dest="plotObserved", action="store_true", help="plotObserved")
parser.add_option("--smooth", dest="smooth", action="store_true", help="smooth")
parser.add_option("--mode", dest="mode", default="nnMLP21", type="string", action="store", help="mode: full or asymptotic")
...
def readResFile(fpath):
  ...
  #        limit["{0:.3f}".format(round(qE.GetValue(),3))] = preFac*l.GetValue()
    ...
    if fpath[-4:]=='.pkl':
      print fpath
      res = pickle.load(file(fpath))
      limit={}
      for l in ['0.025', '0.160', '0.500', '0.840', '0.975', '-1.000']:
        arg = mode[2:]
        if res[arg]['result'].has_key(l):
          limit [l] = res[arg]['result'][l]
      return limit
...
plotObserved = options.plotObserved
doInterpolate = True #options.interpolate
...
##prefix = "asymptotic_T1tttt-madgraph_estimationModifier_09"
    prefix = "asymptotic_T1tttt-madgraph"
  elif mode[:2]=="nn":
    inputpath = "/data/schoef/nnAnalysis/limits/BP_eff05/"
    prefix = "RA4_4j_met100_mt2w200_NormDeco_5000_sigmoid_BP_eff05_"+mode[2:]
...
for tfile in allfiles:
  if mode=="asymptotic":
    if tfile.count("higgsCombine_Asymptotic"):
        resfiles.append(tfile)
  elif mode[:2]=="nn":
    if tfile.count("_limits.pkl"):
        resfiles.append(tfile)
  else:
    if tfile.count("higgsCombineTest.HybridNew.mH120"):
        resfiles.append(tfile)
...
for key in limits.keys():
  ...
  try:
    if plotObserved:
      ...
      h_limit.SetBinContent(bin,xsec*limits[key]["0.500"])
...
#    h_cont_th_up = R.rebin(R.rebin(R.rebin(R.rebin(R.rebin(R.interpolate(h_cont_th_up,"SW"), "SW"),"SW"),"SW"),"SW"),"SW")
    ...
    h_limit = R.interpolate(h_limit, "SW")
    if plotObserved:
      h_cont_obs = R.interpolate(h_cont_obs, "SW")
    h_cont_exp = R.interpolate(h_cont_exp, "SW")
    h_cont_exp_down = R.interpolate(h_cont_exp_down, "SW")
    h_cont_exp_up = R.interpolate(h_cont_exp_up, "SW")
    h_cont_th_down = R.interpolate(h_cont_th_down, "SW")
    h_cont_th_up = R.interpolate(h_cont_th_up, "SW")
        
  if sms=="T1t1t":
    print "Smoothing T1t1t!"
    h_limit = R.interpolate(h_limit, "EW")
    if plotObserved:
      h_cont_obs = R.interpolate(h_cont_obs, "EW")
    h_cont_exp = R.interpolate(h_cont_exp, "EW")
    h_cont_exp_down = R.interpolate(h_cont_exp_down, "EW")
    h_cont_exp_up = R.interpolate(h_cont_exp_up, "EW")
    h_cont_th_down = R.interpolate(h_cont_th_down, "EW")
    h_cont_th_up = R.interpolate(h_cont_th_up, "EW")

  if doSmooth:
      h_limit = R.doSmooth(h_limit,1)
      if plotObserved:
        h_cont_obs = R.doSmooth(h_cont_obs,1)
      h_cont_exp = R.doSmooth(h_cont_exp,1)
      h_cont_exp_down = R.doSmooth(h_cont_exp_down,1)
      h_cont_exp_up = R.doSmooth(h_cont_exp_up,1)
      h_cont_th_down = R.doSmooth(h_cont_th_down,1)
      h_cont_th_up = R.doSmooth(h_cont_th_up,1)
#  else:
#      h_limit = h_limit.Clone()
#      if plotObserved:
#        h_cont_obs = h_cont_obs.Clone()
#      h_cont_exp = h_cont_exp.Clone()
#      h_cont_exp_down = h_cont_exp_down.Clone()
#      h_cont_exp_up = h_cont_exp_up.Clone()
#      h_cont_th_down = h_cont_th_down.Clone()
#      h_cont_th_up = h_cont_th_up.Clone()

  hists = [ h_cont_exp, h_cont_exp_down, h_cont_exp_up, h_cont_th_down, h_cont_th_up]
  if plotObserved:
    hists+=h_cont_obs
  for h in hists:
