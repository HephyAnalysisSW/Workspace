...
preprefix = 'QCDestimation/LpStudy_tt'
...
#        canv.Print(wwwDir+presel+SRname+'_subBkg.pdf')
        ...
        #mergeCanv.SetLogy()
        leg = ROOT.TLegend(0.65,0.85,0.95,0.95)
        leg.SetFillColor(0)
        leg.SetBorderSize(1)
...
#          histos['merged_EWK']['Selection'].Scale(1./histos['merged_EWK']['Selection'].Integral())             
        ...
        leg.AddEntry(histos['merged_EWK']['Selection'],'tt + jets','l')
...
#        histos['merged_EWK']['antiSelection'].Draw('hist same')
        histos['merged_EWK']['Selection'].Draw('hist')
