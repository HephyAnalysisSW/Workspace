...
for sample in allSamples:
  ...
  #    print "Counts = ",getValue(d,"uint_EventCounter_runCounts_PAT.obj")
    ...
    n = c.GetEntries(commoncf+"&&ngoodMuons==1&&ngoodElectrons==0&&nvetoMuons==1")
    res[sample["name"]][3]+=weight*n 
    niceName["3"] ="$==0$ veto $\mu$"  
    print niceName["3"], n,"(",weight*n,")",round(100*weight*n/(basis[sample["name"]]),2),"%"

    n = c.GetEntries(commoncf+"&&ngoodMuons==1&&ngoodElectrons==0&&nvetoMuons==1&&nvetoElectrons==0")
    res[sample["name"]][4]+=weight*n 
    niceName["4"] ="$==0$ veto e" 
    print niceName["4"], n,"(",weight*n,")",round(100*weight*n/(basis[sample["name"]]),2),"%"

    n = c.GetEntries(commoncf+"&&"+"jet0pt>40&&ngoodMuons==1&&ngoodElectrons==0&&nvetoMuons==1&&nvetoElectrons==0")
    res[sample["name"]][5]+=weight*n 
    niceName["5"] ="$>=1$ Jets"  
    print niceName["5"], n,"(",weight*n,")",round(100*weight*n/(basis[sample["name"]]),2),"%"

    n = c.GetEntries(commoncf+"&&"+"jet1pt>40&&ngoodMuons==1&&ngoodElectrons==0&&nvetoMuons==1&&nvetoElectrons==0")
    res[sample["name"]][6]+=weight*n 
    niceName["6"] ="$>=2$ Jets"  
    print niceName["6"], n,"(",weight*n,")",round(100*weight*n/(basis[sample["name"]]),2),"%"

    n = c.GetEntries(commoncf+"&&"+"jet2pt>40&&ngoodMuons==1&&ngoodElectrons==0&&nvetoMuons==1&&nvetoElectrons==0")
    res[sample["name"]][7]+=weight*n 
    niceName["7"] ="$>=3$ Jets"  
    print niceName["7"], n,"(",weight*n,")",round(100*weight*n/(basis[sample["name"]]),2),"%"

    n = c.GetEntries(commoncf+"&&"+"ht>300&&jet2pt>40&&ngoodMuons==1&&ngoodElectrons==0&&nvetoMuons==1&&nvetoElectrons==0")
    res[sample["name"]][8]+=weight*n 
    niceName["8"] ="\HT>=300 \GeV"  
    print niceName["8"], n,"(",weight*n,")",round(100*weight*n/(basis[sample["name"]]),2),"%"

    n = c.GetEntries(commoncf+"&&"+"barepfmet>100&&ht>300&&jet2pt>40&&ngoodMuons==1&&ngoodElectrons==0&&nvetoMuons==1&&nvetoElectrons==0")
  ...
  #  print niceName["4"], n,"(",weight*n,")",round(100*weight*n/(basis[sample["name"]]),2),"%"
    ...
    n = c.GetEntries(commoncf+"&&"+"jet2pt>40&&ngoodMuons==1&&ngoodElectrons==0&&nvetoMuons==1&&nvetoElectrons==0&&(!(btag0>=1.74))")
    res[sample["name"]][10]+=weight*n 
    niceName["10"] ="$==0$ bjets" 
    print niceName["10"], n,"(",weight*n,")",round(100*weight*n/(basis[sample["name"]]),2),"%"

    n = c.GetEntries(commoncf+"&&"+"jet2pt>40&&ngoodMuons==1&&ngoodElectrons==0&&nvetoMuons==1&&nvetoElectrons==0&&((btag0>=1.74)&&(!(btag1>=1.74)))")
    res[sample["name"]][11]+=weight*n 
    niceName["11"] ="$==1$ bjets" 
    print niceName["11"], n,"(",weight*n,")",round(100*weight*n/(basis[sample["name"]]),2),"%"

    n = c.GetEntries(commoncf+"&&"+"jet2pt>40&&ngoodMuons==1&&ngoodElectrons==0&&nvetoMuons==1&&nvetoElectrons==0&&(btag1>=1.74)")
