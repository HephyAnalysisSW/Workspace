...
def getT1tttt(mgl, mn, cdir = "v19"):
  res = copy.deepcopy(mc)
  res["dirname"] = ["/data/adamwo/convertedTuples_"+cdir+"/copyMET/Ele/", "/data/adamwo/convertedTuples_"+cdir+"/copyMET/Mu/"]
...
for sample in allSamples:
  sample["dirname"] = ["/data/schoef/convertedTuples_v19/copyMET/Ele/", "/data/schoef/convertedTuples_v19/copyMET/Mu/"]
...
if not doOnlyMET:
  ...
  jet0_pt_stack                     = getStack(":jet0pt;p_{T} of leading jet (GeV);Number of Events / 50 GeV",[38,40,1940], commoncf, signals, topIsLargest)
  jet0_pt_stack[0].data_histo.GetXaxis().SetLabelSize(0.04)
  ...
  ngoodElectrons_stack = getStack("ngoodElectrons :ngoodElectrons;Number of electrons;Number of Events",[5,0-.5,5-.5], commoncf.replace("(singleElectronic&&nvetoElectrons==1&&nvetoMuons==0||singleMuonic&&nvetoElectrons==0&&nvetoMuons==1)","ngoodMuons==0"), signals, topIsLargest)
  ngoodElectrons_stack[0].data_histo.GetXaxis().SetNdivisions(505)
  ngoodElectrons_stack[0].addOverFlowBin = "upper"
  allStacks.append(ngoodElectrons_stack)

  ngoodMuons_stack = getStack("ngoodMuons :ngoodMuons;Number of muons;Number of Events",[5,0-.5,5-.5], commoncf.replace("(singleElectronic&&nvetoElectrons==1&&nvetoMuons==0||singleMuonic&&nvetoElectrons==0&&nvetoMuons==1)", "ngoodElectrons==0"), signals, topIsLargest)
  ngoodMuons_stack[0].data_histo.GetXaxis().SetNdivisions(505)
