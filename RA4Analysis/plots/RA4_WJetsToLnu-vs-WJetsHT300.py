...
for sample in allSamples:
  sample["Chain"] = "pfRA4Analyzer/Events"

def getStack(varstring, binning, cutstring = True, varfunc = ""):
  compareSample = wjetsHT300 
  MCSample = wjets

  PYTHIA          = variable(varstring, binning, cutstring)
  PYTHIA.sample   = compareSample 
  PYTHIA.color    = dataColor
  PYTHIA.legendText= compareSample["name"]
  PYTHIA.style              = "l02"
...
genmet_stack = getStack(":genmet;#slash{E}_{T} (GeV);Number of Events / 25 GeV",[21,0,525], commoncf)
genmet_stack[0].addOverFlowBin = "upper"
allStacks.append(genmet_stack)

if not doOnlyMET:

  jet0_pt_stack                     = getStack(":jet0pt;p_{T} of leading jet (GeV);Number of Events / 30 GeV",[27,10,820], commoncf)
  jet0_pt_stack[0].addOverFlowBin = "upper"
  allStacks.append(jet0_pt_stack)
  jet1_pt_stack                     = getStack(":jet1pt;p_{T} of 2^{nd} leading jet (GeV);Number of Events / 30 GeV",[27,10,810], commoncf)
  jet1_pt_stack[0].addOverFlowBin = "upper"
  allStacks.append(jet1_pt_stack)
  jet2_pt_stack                     = getStack(":jet2pt;p_{T} of 3^{rd} leading jet (GeV);Number of Events / 30 GeV",[27,10,810], commoncf)
  jet2_pt_stack[0].addOverFlowBin = "upper"
  allStacks.append(jet2_pt_stack)
  jet3_pt_stack                     = getStack(":jet3pt;p_{T} of 4^{th} leading jet (GeV);Number of Events / 30 GeV",[27,10,810], commoncf+"&&jet3pt>0")
  jet3_pt_stack[0].addOverFlowBin = "upper"
  allStacks.append(jet3_pt_stack)

  kinMetSig_stack                   = getStack(":kinMetSig;S_{MET};Number of Events",[25,0,25], commoncf)
  kinMetSig_stack[0].addOverFlowBin = "upper"
  allStacks.append(kinMetSig_stack)

  mT_stack  = getStack(":mT;m_{T} (GeV);Number of Events / 20 GeV",[27,0,540], commoncf)
  mT_stack[0].addOverFlowBin = "upper"
  allStacks.append(mT_stack)

  ht_stack                          = getStack(":ht;H_{T} (GeV);Number of Events / 50 GeV",[35,0,1750], commoncf)
  ht_stack[0].addOverFlowBin = "upper"
  allStacks.append(ht_stack)

  m3_stack = getStack(":m3;M_{3} (GeV);Number of Events / 30 GeV",[51,0,1530], commoncf)
  m3_stack[0].addOverFlowBin = "upper"
  allStacks.append(m3_stack)

  lepton_pt_stack = getStack("p_{T} (GeV):lepton_pt;p_{T,lep.} (GeV);Number of Events / 20 GeV",[26,0,520], commoncf)
  lepton_pt_stack[0].addOverFlowBin = "upper"
  allStacks.append(lepton_pt_stack)

  lepton_eta_stack                  = getStack(":lepton_eta;|#eta^{#mu}|;Number of Events",[62,-3.1,3.1], commoncf)
  lepton_eta_stack[0].addOverFlowBin = "both"
  allStacks.append(lepton_eta_stack)
  
  ngoodElectrons_stack = getStack("ngoodElectrons :ngoodElectrons;Number of electrons;Number of Events",[5,0,5], commoncf.replace("singleMuonic&&nvetoMuons==1&&nvetoElectrons==0", "ngoodMuons==1&&nvetoMuons==1"))
  ngoodElectrons_stack[0].addOverFlowBin = "upper"
  allStacks.append(ngoodElectrons_stack)

  ngoodMuons_stack = getStack("ngoodMuons :ngoodMuons;Number of muons;Number of Events",[5,0,5], commoncf.replace("singleMuonic&&nvetoMuons==1", "ngoodMuons>0"))
