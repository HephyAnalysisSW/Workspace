...
def getT1tttt(mgl, mn, cdir = "v15"):
  ...
  res["dirname"] = "/data/adamwo/convertedTuples_"+cdir+"/copyMET/Mu/"
...
if useConvertedTuples:
  for sample in allSamples:
    sample["dirname"] = "/data/schoef/convertedTuples_v18/copyMET50/Mu/"
    targetLumi = 20000.
  ...
  if mode=="singleLepton":
    data["dirname"] = "/data/schoef/convertedTuples_v18/copyMET50/Mu/"
...
presel = "pf-6j40"
preprefix     = "singleMu-ht1000-met50-250-bt2-trigger"
#additionalCut = "met>100&&nbtags>=2&&ht>300"
#additionalCut = "met>150&&ht>400&&ht<450&&nbtags==0"
additionalCut = "ht>1000&&type1phiMet>50&&type1phiMet<250&&leptonPt>30&&nbtags>=2"
additionalDataCut = "HLTIsoMu24eta2p1"
...
if presel == "pf-3j40":
  chainstring = "Events"
  commoncf = "jet2pt>40&&leptonPt>20&&(singleMuonic&&nvetoMuons==1&&nvetoElectrons==0)"
if presel == "pf-4j40":
  chainstring = "Events"
  commoncf = "jet3pt>40&&leptonPt>20&&singleMuonic&&nvetoMuons==1&&nvetoElectrons==0"
if presel == "pf-6j40":
  chainstring = "Events"
  commoncf = "njets>=6&&leptonPt>20&&singleMuonic&&nvetoMuons==1&&nvetoElectrons==0"
if presel == "pf-ex3j40":
  chainstring = "Events"
  commoncf = "njets==3&&leptonPt>20&&singleMuonic&&nvetoMuons==1&&nvetoElectrons==0"
if presel == "pf-3-5j40":
  chainstring = "Events"
  commoncf = "njets>=3&&njets<=5&&leptonPt>20&&singleMuonic&&nvetoMuons==1&&nvetoElectrons==0"
if presel == "pf-ex4j40":
  chainstring = "Events"
  commoncf = "njets==4&&leptonPt>20&&singleMuonic&&nvetoMuons==1&&nvetoElectrons==0"
if presel == "pf-ex5j40":
  chainstring = "Events"
  commoncf = "njets==5&&leptonPt>20&&singleMuonic&&nvetoMuons==1&&nvetoElectrons==0"
...
def getStack(varstring, binning, cutstring, signals, topIsLargest = False, varfunc = ""):
  if additionalDataCut !="":
    cutstringData = cutstring+"&&"+additionalDataCut
  else:
    cutstringData = cutstring
  DATA          = variable(varstring, binning, cutstringData)
  DATA.sample   = data
...
#    var.legendCoordinates=[0.64,0.95 - 0.05*nhistos,.98,.95] #for signals = [0,1]
  res.append(DATA)
  if varfunc!="":
    for var in res:
      var.varfunc = varfunc
...
if not doOnlyMET:
  ...
  rawmet_stack  = getStack(":rawmet; (GeV);Number of Events / 20 GeV",[27,0,540], commoncf, signals, topIsLargest, rawMET)
  ...
  btag0pt_stack                     = getStack(":btag0pt;p_{T} of leading b-jet (GeV);Number of Events / 40 GeV",[26,0,1040], commoncf, signals, topIsLargest)
  btag0pt_stack[0].addOverFlowBin = "upper"
  allStacks.append(btag0pt_stack)
  btag1pt_stack                     = getStack(":btag1pt;p_{T} of 2^{nd} leading b-jet (GeV);Number of Events / 40 GeV",[26,0,1040], commoncf, signals, topIsLargest)
  btag1pt_stack[0].addOverFlowBin = "upper"
  allStacks.append(btag1pt_stack)

  jet0_pt_stack                     = getStack(":jet0pt;p_{T} of leading jet (GeV);Number of Events / 40 GeV",[26,0,1040], commoncf, signals, topIsLargest)
  jet0_pt_stack[0].addOverFlowBin = "upper"
  allStacks.append(jet0_pt_stack)
  jet1_pt_stack                     = getStack(":jet1pt;p_{T} of 2^{nd} leading jet (GeV);Number of Events / 40 GeV",[26,0,1040], commoncf, signals, topIsLargest)
  jet1_pt_stack[0].addOverFlowBin = "upper"
  allStacks.append(jet1_pt_stack)
  jet2_pt_stack                     = getStack(":jet2pt;p_{T} of 3^{rd} leading jet (GeV);Number of Events / 40 GeV",[21,0,840], commoncf, signals, topIsLargest)
  jet2_pt_stack[0].addOverFlowBin = "upper"
  allStacks.append(jet2_pt_stack)
  jet3_pt_stack                     = getStack(":jet3pt;p_{T} of 4^{th} leading jet (GeV);Number of Events / 40 GeV",[21,00,840], commoncf+"&&jet3pt>0", signals, topIsLargest)
  ...
  m3_stack = getStack(":m3;M_{3} (GeV);Number of Events / 100 GeV",[26, 0, 2600], commoncf, signals, topIsLargest)
  ...
  ngoodElectrons_stack = getStack("ngoodElectrons :ngoodElectrons;Number of electrons;Number of Events",[5,0-.5,5-.5], commoncf.replace("singleMuonic&&nvetoMuons==1&&nvetoElectrons==0", "ngoodMuons==1&&nvetoMuons==1"), signals, topIsLargest)
  ngoodElectrons_stack[0].addOverFlowBin = "upper"
  allStacks.append(ngoodElectrons_stack)

  ngoodMuons_stack = getStack("ngoodMuons :ngoodMuons;Number of muons;Number of Events",[5,0-.5,5-.5], commoncf.replace("singleMuonic&&nvetoMuons==1", "ngoodMuons>0"), signals, topIsLargest)
  ngoodMuons_stack[0].addOverFlowBin = "upper"
  allStacks.append(ngoodMuons_stack)

  nvetoElectrons_stack = getStack("nvetoElectrons :nvetoElectrons;Number of veto electrons;Number of Events",[5,0-.5,5-.5], commoncf.replace("singleMuonic&&nvetoMuons==1&&nvetoElectrons==0","ngoodMuons==1&&nvetoMuons==1"), signals, topIsLargest)
  nvetoElectrons_stack[0].addOverFlowBin = "upper"
  allStacks.append(nvetoElectrons_stack)

  nvetoMuons_stack = getStack("nvetoMuons :nvetoMuons;Number of veto muons;Number of Events",[5,0-.5,5-.5], commoncf.replace("&&nvetoMuons==1",""), signals, topIsLargest)
...
#  allStacks.append(nbtags_rw_stack)
  ...
  nljets_stack = getStack(":XXX;Number of untagged Jets;Number of Events",[20,0,20], commoncf, signals, topIsLargest, nljets)
  nljets_stack[0].addOverFlowBin = "upper"
  allStacks.append(nljets_stack)
...
if not doOnlyMET:
  btag0_stack                   = getStack(":btag0;highest SSV_{HE};Number of Events",[30,-1,6.], commoncf, signals, topIsLargest)
...
#  drawNMStacks(1,1,[ht2_stack],             subdir+prefix+"ht2", False)
  ...
  drawNMStacks(1,1,[nljets_stack],             subdir+prefix+"nljets", False)
...
#  drawNMStacks(1,1,[ht2_stack],             subdir+prefix+"ht2_lin", False)
  drawNMStacks(1,1,[btag0pt_stack],         subdir+prefix+"btag0pt_lin", False)
  drawNMStacks(1,1,[btag1pt_stack],         subdir+prefix+"btag1pt_lin", False)
