...
#          "+/-",round(sqrt(smallVar),2), "(PU+other)"
  ...
  for logy in [ True, False]:
    ...
    hObs_2b [htb].GetYaxis().SetRangeUser(0.07*logy*10**(-2.5*logy), 10**(2*logy)*1.5*0.22)
...
for htb in [(400,2500), (750,2500), (1000,2500)]:
  ...
  for i, metb in enumerate(metbins_3b):
    ...
    hObs_3b [htb].SetBinContent(b, obs/(binwidth))
    hObs_3b [htb].SetBinError(b, sqrt(obs)/(binwidth))
    hObs_3b [htb].SetMarkerColor(ROOT.kBlack)
    hObs_3b [htb].SetMarkerStyle(20)
    hObs_3b [htb].SetLineColor(ROOT.kBlack)
    hPred_3b[htb].SetBinContent(b, pred/(binwidth))
  ...
  for logy in [ True, False]:
    ...
    c1 = ROOT.TCanvas()
    c1.SetLogy(logy)
    hObs_3b [htb].GetYaxis().SetRangeUser(0.007*logy*10**(-2*logy), 10**logy*1.8*0.51)
    if htb==(1000,2500):
      hObs_3b [htb].GetYaxis().SetRangeUser(0.007*logy*10**(-2*logy), 10**logy*2.0*0.51)
