...
for srNJet in sorted(SR):
  ...
  for stb in sorted(SR[srNJet]):
    ...
    for htb in sorted(SR[srNJet][stb]):
      ...
      #fratio_diLep = ROOT.TFile(path+'st250_ht500_njet3_nbtagEq0_nJet30_diLep_Ratio.root')
      #fratio_oneLep = ROOT.TFile(path+'st250_ht500_njet3_nbtagEq0_nJet30_Ratio.root')
      fratio_diLep = ROOT.TFile(path+Name+'_'+p['varname']+'_diLep_ISR_Ratio.root')
      fratio_oneLep = ROOT.TFile(path+Name+'_'+p['varname']+'_ISR_Ratio.root')
      fratio_diLep.ls()
      fratio_oneLep.ls()
