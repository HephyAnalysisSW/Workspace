...
#signalRegions = {(6, 7):{(450, -1):{(500,1000):{"deltaPhi":0.75}}}}
...
for srNJet in sorted(signalRegions):
  ...
  for stb in sorted(signalRegions[srNJet]):
    ...
    for htb in sorted(signalRegions[srNJet][stb]):
      ...
      name_bla, MB_cut_jec_central   = nameAndCut(stb, htb, srNJet, btb=(0,-1), presel=presel+"&&deltaPhi_Wl>"+str(deltaPhiCut), btagVar = "jec_nBJet_central" , stVar = 'jec_LT_central', htVar = 'jec_ht_central', njetVar='jec_nJet_central')
      name_bla, MB_cut_jec_up        = nameAndCut(stb, htb, srNJet, btb=(0,-1), presel=presel+"&&deltaPhi_Wl>"+str(deltaPhiCut), btagVar = "jec_nBJet_up"      , stVar = 'jec_LT_up'     , htVar = 'jec_ht_up'     , njetVar='jec_nJet_up')
      ...
      #for mglu in [1000] :
        ...
        #for mlsp in [100] :
          ...
          sig_dict[srNJet][stb][htb]["signals"][mglu][mlsp]["delta_jec"] = 0
...
pickle.dump(sig_dict,file('/data/easilar/Results2016/ICHEP/signal_Spring16/mglu'+str(mglu)+'Signals_JECuncUpdated_12p88_pkl','w'))
