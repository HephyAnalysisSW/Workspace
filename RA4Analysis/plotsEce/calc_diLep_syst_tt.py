...
for srNJet in [(8,-1)]:
  ...
  for stb in sorted(signalRegions[srNJet]):
    ...
    for htb in sorted(signalRegions[srNJet][stb]):
      ...
      name_bla, tt_SB_cut = nameAndCut(stb, htb, (4,5), btb=c_tt["cut"], presel=presel, btagVar = btagString)
      name_bla, tt_SB_cut_CR = nameAndCut(stb, htb, (4,5), btb=c_tt["cut"], presel=presel+"&&deltaPhi_Wl<"+str(deltaPhiCut), btagVar = btagString)
      ...
      yield_rest_W_MB_in_CR                     = max(0,getYieldFromChain(cW   , cut_tt_CR, weight = common_weight+"*"+c_tt["weight0b"]))
...
#####
      ...
      yield_rest_W_SB_in_CR                     = max(0,getYieldFromChain(cW   , cut_tt_CR, weight = common_weight+"*"+c_tt["weight0b"]))
