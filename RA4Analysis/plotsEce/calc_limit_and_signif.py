...
path = "/afs/hephy.at/user/e/easilar/www/PHYS14v3/limit_results/singleMuonic/combined_bin/"
...
for lum in lumi_bins:
  ...
  for i , bin in enumerate(found_bin):
    ...
    signals = [
              {'name': 's1500' ,'value': float(bin['S1500'])/float(lumi_origin) , 'label': 'T5q^{4} 1.5_0.8_0.1'}, \
              {'name': 's1200' ,'value': float(bin['S1200'])/float(lumi_origin) , 'label': 'T5q^{4} 1.2_1.0_0.8'}, \
             ]
    signal = signals[0]
...
#latex.DrawLatex(0.55,0.31,"bkg yield:"+str(round(bkg_Y*4,2)))
#latex.DrawLatex(0.55,0.35,"signal yield:"+str(round(signal['value']*4,2)))
...
can.SaveAs(path+"significance_1.5_0.8_0.1_"+str(len(search_bins))+".png")
