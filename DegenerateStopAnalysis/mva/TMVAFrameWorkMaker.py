...
prefix_dataset = 'test_DegenerateStop_BkgMix'
setup = pickle.load(file('/data/schoef/DegenerateStop/datasets/'+prefix_dataset+'.pkl'))
...
setup['fomPlotZoomCoordinates'] = [0, 0.99, 0.3, 1.0]
setup['mvaInputObs'] = ['met_pt', 'nBJetMedium25', 'htJet25', 'nJet', 'leptonPt', 'mT']

prefix_observables = '_'.join(setup['mvaInputObs'])

prefix = '_'.join([prefix_dataset, prefix_observables])
setup['TMVAOutputFile'] = '/data/'+nfsUser+'/DegenerateStop/TMVAAnalyzers/'+prefix+'.root'
setup['weightDir'] = '/data/'+nfsUser+'/DegenerateStop/TMVAAnalyzers/'+prefix+'/'
