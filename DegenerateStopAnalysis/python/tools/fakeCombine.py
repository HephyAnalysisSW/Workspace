...
import Workspace.DegenerateStopAnalysis.tools.limitTools as limitTool
from   Workspace.HEPHYPythonTools.asym_float import asym_float 
import os
import glob
import pickle

def getFakeEstimateFromCombine( obs, exp, sig=1, bin_name = "bin1", systs=None ):
    print bin_name
    cfw        = CombinedCard();
    ...
    cfw.specifyObservations( ylds , obsProcess = "obs" , bins = [bin_name] ) 
    ...
    rmax = 2* ( (obs-exp).val + (obs-exp).sigma)
    rMaxOpt=""
    if rmax>20:
        rMaxOpt="--rMax=%s"%rmax
    print '\n\n\n', '------', obs, exp, obs-exp, rmax
    ...
    if youAreFancy:
        ...
        output = {'cen': cen , 'down': down, 'up':up } 
    return output  
...
def getFakeEstimateParal( prompt_fake_yields, bins = None, data_key = "__Data_X_TL", prompt_key="__Prompts_X_TL", nProc = 20):
    bins = bins if bins else prompt_fake_yields.keys()
    args = []
    for b in bins:
        args.append( [ prompt_fake_yields[b][data_key] , prompt_fake_yields[b][prompt_key] , 1, b] )
    res = degTools.runFuncInParal( getFakeEstimateFromCombineWrapper , args , nProc = nProc) 
    return dict(zip( bins, res))
