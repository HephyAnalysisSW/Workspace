...
\BLOCK{- set colSetup="|c| c|"+"c"*(nCols) +" | " }
...
&   \BLOCK{ for col in latexCols }  &  \VAR{ col}   \BLOCK{endfor } \\ 
...
& \VAR{row | fix }  \BLOCK{ for col in colList }  &  \VAR{ yieldDict[row][col] | replace("+-","$\pm$") }   \BLOCK{endfor } \\ 
