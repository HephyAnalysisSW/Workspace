...
select("Jet_pt[0]", cuts['ISR'], ">") + "&&" + \
select("Jet_eta", cuts['eta'], "<") + ")",
'Gen' : \
str(weight) + "*(" + \
select("met_genPt", cuts['MET'], ">") + "&&" + \
select("GenJet_pt[0]", cuts['ISR'], ">") + "&&" + \
select("GenJet_eta", cuts['eta'], "<") + ")"
})
...
variable = "met_pt"
h1 = drawhist(T2DegSample, variable, selection['Reco'])
h1.SetName("h1")
...
variable = "met_genPt"
h2 = drawhist(T2DegSample, variable, selection['Gen'])
h2.SetName("h2")
h2.Draw("same")
h2.SetFillColor(0)
...
variable = "Jet_pt"
h3 = drawhist(T2DegSample, variable, selection['Reco'])
h3.SetName("h3")
...
variable = "GenJet_pt"
h4 = drawhist(T2DegSample, variable, selection['Gen'])
h4.SetName("h4")
h4.Draw("same")
h4.SetFillColor(0)
