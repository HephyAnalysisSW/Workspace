...
if 'sr1' in region: etaBins = ['eta_lt_1p5']
elif 'sr2' in region: etaBins = ['eta_lt_1p5', 'eta_gt_1p5']
...
if doPlots:
   ...
   for l in ['el', 'mu']:
      for etaBin in etaBins:
         if l == "el":   lepName = "Electron"
         elif l == "mu": lepName = "Muon"

         estDir[l][etaBin] = "%s/%s/log/root/"%(baseFileDir, etaBin)
         estDir[l][etaBin] = estDir[l][etaBin].replace(lepton, lepName)

         #estFileName[etaBin] = "closure_%s_%s_%s%s.root"%(lep, region, measurementType, suffix2)
         estFileName[l][etaBin] = "fakesEstimate_%s_%s_%s%s.root"%(l, region, measurementType, suffix2)
         
         if 'lt' in etaBin:   etaName = 'plus_lepEta1p5'
         elif 'gt' in etaBin: etaName = 'plus_lepEta_gt_1p5'
        
         if 'sr1' in region: 
            canvName = 'Canvas_TTX_%s_plus_highWeightVeto_plus_notTight_plus_fake_Leptonp_TPlot'%(regDefs['regDef'].replace('_plus_highWeightVeto', ''))
         elif 'sr2' in region:
            canvName = 'Canvas_TTX_%s_%s_plus_highWeightVeto_plus_notTight_plus_fake_Leptonp_TPlot'%(regDefs['regDef'].replace('_plus_highWeightVeto', ''), etaName)

         if 'data' in measurementType: canvName = canvName.replace('TTX', 'DataBlind')
         plotName = "fakesEstimate"
         
         try:
            estFile[l][etaBin] = ROOT.TFile(estDir[l][etaBin] + estFileName[l][etaBin])
            estimate[l][etaBin] = estFile[l][etaBin].Get(canvName).GetPrimitive(canvName + '_p1').GetPrimitive(plotName).Clone()
         except:
            AttributeError
            print "File", estDir[l][etaBin] + estFileName[l][etaBin], "not found. Exiting."
            sys.exit()
            #continue
   ...
   #binsMu = array('d', bins['pt'])
   ##est_el = estimate['el'].Rebin(len(bins['pt'])-1, "fakesEstimate_el_rebinned", binsMu)
   #est_mu = estimate['mu'].Rebin(len(bins['pt'])-1, "fakesEstimate_mu_rebinned", binsMu)

   #estimate['mu'] = ROOT.TH1D()
   #estimate['mu'].Copy(est_mu)
   #est_el.Copy(estimate['el'])
   ...
   for etaBin in etaBins:
      ...
      est_el[etaBin] = estimate['mu'][etaBin].Clone()
      est_el[etaBin].Reset()

      n = est_el[etaBin].GetNbinsX()

      for i in range(n):
         i += 1
         est_el[etaBin].SetBinContent(i+1, estimate['el'][etaBin].GetBinContent(i))
         est_el[etaBin].SetBinError(i+1, estimate['el'][etaBin].GetBinError(i))
   
      est[etaBin] = est_el[etaBin] + estimate['mu'][etaBin] # final estimate is sum of estimates from two eta bins 
 
   #estList = ROOT.TList()
   #estList.Add(estimate['el'])
   #estList.Add(estimate['mu'])
   #
   #finalHists['estimate'] = estimate['mu'].Clone()
   #finalHists['estimate'].Reset()
   #finalHists['estimate'].Merge(estList)
   
   finalHists['estimate'] = est['eta_lt_1p5'] + est['eta_gt_1p5'] # final estimate is sum of estimates from two eta bins 
