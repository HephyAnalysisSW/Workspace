...
plot = "eleID"
noIsoFile = ROOT.TFile(filedir + "/noIso/efficiency/root/eleIDeff_noIso_" + inputSample + z + ".root", "read")
isoFile = ROOT.TFile(filedir + "/iso/" + isolation + "/efficiency/root/eleIDeff_iso_" + inputSample + z + ".root", "read")
for WP in WPs:
   total[plot][WP] = noIsoFile.Get("c1").GetPrimitive("c1_1").GetPrimitive(plot + "_" + WP) 
   passed[plot][WP] = isoFile.Get("c1").GetPrimitive("c1_1").GetPrimitive(plot + "_" + WP) 

plot = "misID"
noIsoFile = ROOT.TFile(filedir + "/noIso/misID/root/eleMisID_noIso_" + inputSample + z + ".root", "read")
isoFile = ROOT.TFile(filedir + "/iso/" + isolation + "/misID/root/eleMisID_iso_" + inputSample + z + ".root", "read")
for WP in WPs:
   total[plot][WP] = noIsoFile.Get("c1").GetPrimitive("c1_1").GetPrimitive(plot + "_" + WP) 
   passed[plot][WP] = isoFile.Get("c1").GetPrimitive("c1_1").GetPrimitive(plot + "_" + WP) 

plot = "misID2"
noIsoFile = ROOT.TFile(filedir + "/noIso/misID2/root/eleMisID2_noIso_" + inputSample + z + ".root", "read")
isoFile = ROOT.TFile(filedir + "/iso/" + isolation + "/misID2/root/eleMisID2_iso_" + inputSample + z + ".root", "read")
for WP in WPs:
   total[plot][WP] = noIsoFile.Get("c1").GetPrimitive("c1_1").GetPrimitive(plot + "_" + WP) 
   passed[plot][WP] = isoFile.Get("c1").GetPrimitive("c1_1").GetPrimitive(plot + "_" + WP) 
...
for plot in sorted(total.keys()):
   for WP in WPs: 
      effs[plot][WP] = ROOT.TEfficiency(passed[plot][WP], total[plot][WP]) #(passed, total) 
...
for i,plot in enumerate(sorted(total.keys())):
   ...
   if plot == "eleID": effs[plot]['Veto'].SetTitle("Efficiency of " + isolation + " Isolation Cut (" + inputSample + " Sample) ; Generated Electron p_{T} / GeV ; Efficiency")
   elif plot == "misID": effs[plot]['Veto'].SetTitle("MisID Efficiency of " + isolation + " Isolation Cut (" + inputSample + " Sample) ; Reconstructed Electron p_{T} / GeV ; Efficiency")
   elif plot == "misID2": effs[plot]['Veto'].SetTitle("MisID2 Efficiency of " + isolation + " Isolation Cut (" + inputSample + " Sample) ; Reconstructed Electron p_{T} / GeV ; Efficiency")
   
   effs[plot]['Veto'].SetMarkerColor(ROOT.kGreen+3) 
   effs[plot]['Veto'].SetLineColor(ROOT.kGreen+3) 
   effs[plot]['Veto'].Draw("AP") 
   ROOT.gPad.SetGridx() 
   ROOT.gPad.SetGridy() 
   ROOT.gPad.Modified() 
   ROOT.gPad.Update() 
   effs[plot]['Veto'].GetPaintedGraph().GetXaxis().SetLimits(xmin,xmax) 
   effs[plot]['Veto'].GetPaintedGraph().SetMinimum(0) 
   effs[plot]['Veto'].GetPaintedGraph().SetMaximum(1) 
   effs[plot]['Veto'].GetPaintedGraph().GetXaxis().CenterTitle() 
   effs[plot]['Veto'].GetPaintedGraph().GetYaxis().CenterTitle() 
...
#Write to file
if save == 1:
   savedir = "/afs/hephy.at/user/m/mzarucki/www/plots/electronReconstruction/electronID/iso/" + isolation + "/isoCutEffs" #web address: http://www.hephy.at/user/mzarucki/plots/electronReconstruction/electronIdEfficiency
   ...
   #Save to Web
   c1.SaveAs(savedir + "/isoCutEffs_" + inputSample + z + ".png")
   c1.SaveAs(savedir + "/root/isoCutEffs_" + inputSample + z + ".root")
   c1.SaveAs(savedir + "/pdf/isoCutEffs_" + inputSample + z + ".pdf")
