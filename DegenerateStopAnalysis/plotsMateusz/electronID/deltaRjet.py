...
#Generated electrons
hists['None'] = makeHist(Events, deltaRjet, normFactor + "*" + weight + "*(" + preSel + "&&" + genSel + ")", nbins, xmin, xmax)
hists['None'].SetName("DeltaRjet")
hists['None'].SetTitle("Electron and ISR Jet DeltaR Distributions for Various IDs (" + inputSample + " Sample)")
hists['None'].GetXaxis().SetTitle("DeltaR between Electron and ISR Jet")
...
for i,WP in enumerate(WPs):
   hists[WP] = makeHist(Events, deltaRjet, normFactor + "*" + weight + "*(" + preSel + "&&" + genSel + "&& (" + cutSel + str(i+1) + "))", nbins, xmin, xmax)
   hists[WP].SetName("deltaRjet_" + WP)
...
l1 = makeLegend()
l1.AddEntry("DeltaRjet", "dR(ele,jet)", "F")
l1.AddEntry("deltaRjet_Veto", "Veto ID", "F")
l1.AddEntry("deltaRjet_Loose", "Loose ID", "F")
l1.AddEntry("deltaRjet_Medium", "Medium ID", "F")
l1.AddEntry("deltaRjet_Tight", "Tight ID", "F")
...
if mvaWPs == 1:
   ...
   for WP in WPs.keys():
      ...
      hists[WP] = makeHist(Events, deltaRjet, normFactor + "*" + weight + "*(" + preSel + "&&" + genSel + "&&" + mvaSel + ")", nbins, xmin, xmax)
      hists[WP].SetName("deltaRjet_" + WP)
      hists[WP].SetFillColor(0)
      hists[WP].SetLineWidth(3)
      hists[WP].Draw("histsame")
   ...
   l1.AddEntry("deltaRjet_WP90", "MVA ID (WP90)", "F")
   l1.AddEntry("deltaRjet_WP80", "MVA ID (WP80)", "F")
...
#Write to file
if save == 1:
   if mvaWPs == 1: savedir = "/afs/hephy.at/user/m/mzarucki/www/plots/electronReconstruction/electronID/standard/deltaRjet" #web address: http://www.hephy.at/user/mzarucki/plots/electronReconstruction/electronIdEfficiency
   elif mvaWPs == 0: savedir = "/afs/hephy.at/user/m/mzarucki/www/plots/electronReconstruction/electronID/standard/deltaRjet/noMva"
