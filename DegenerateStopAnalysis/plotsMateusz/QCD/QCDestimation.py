...
for sel in abcd.values(): 
   SRs[sel] = {\
      ...
      'SR1b':["SR1b", combineCuts(btw(variables['eleMt'][1][sel], 60, 95), variables['elePt'][1][sel] + " < 30")],
      'SR1c':["SR1c", combineCuts(variables['eleMt'][1][sel] + " > 95", variables['elePt'][1][sel] + " < 30")],
      ...
      'SRL1b':["SRL1b", combineCuts(btw(variables['eleMt'][1][sel], 60, 95), btw(variables['elePt'][1][sel], 5, 12))],
      'SRH1b':["SRH1b", combineCuts(btw(variables['eleMt'][1][sel], 60, 95), btw(variables['elePt'][1][sel], 12, 20))],
      'SRV1b':["SRV1b", combineCuts(btw(variables['eleMt'][1][sel], 60, 95), btw(variables['elePt'][1][sel], 20, 30))],
      
      'SRL1c':["SRL1c", combineCuts(variables['eleMt'][1][sel] + " > 95", btw(variables['elePt'][1][sel], 5, 12))],
      'SRH1c':["SRH1c", combineCuts(variables['eleMt'][1][sel] + " > 95", btw(variables['elePt'][1][sel], 12, 20))],
      'SRV1c':["SRV1c", combineCuts(variables['eleMt'][1][sel] + " > 95", btw(variables['elePt'][1][sel], 20, 30))]
   }
...
if estimation: 
   ...
   for reg in regions:
      yields[reg] = {}
      for sel in abcd:
         yields[reg][sel] = Yields(samples, samplesList, QCD[reg][sel], cutOpt = "combinedList", weight = "weight", pklOpt = False, tableName = reg + "_" + sel, nDigits = 2, err = True, verbose = True, nSpaces = 10)
      
      if yields[reg]['IXA'].yieldDictFull['qcd']['QCD_IXA_' + reg].val:
         
         if ABCD == "1" or ABCD == "4" or ABCD == "5": #3D ABCD
            QCD_IX_A[reg] = yields[reg]['IX_A'].yieldDictFull['dblind']['QCD_IX_A_' + reg] - yields[reg]['IX_A'].yieldDictFull['w']['QCD_IX_A_' + reg] - yields[reg]['IX_A'].yieldDictFull['tt']['QCD_IX_A_' + reg] - yields[reg]['IX_A'].yieldDictFull['z']['QCD_IX_A_' + reg]
            QCD_XA_I[reg] = yields[reg]['XA_I'].yieldDictFull['dblind']['QCD_XA_I_' + reg] - yields[reg]['XA_I'].yieldDictFull['w']['QCD_XA_I_' + reg] - yields[reg]['XA_I'].yieldDictFull['tt']['QCD_XA_I_' + reg] - yields[reg]['XA_I'].yieldDictFull['z']['QCD_XA_I_' + reg]
            QCD_IA_X[reg] = yields[reg]['IA_X'].yieldDictFull['dblind']['QCD_IA_X_' + reg] - yields[reg]['IA_X'].yieldDictFull['w']['QCD_IA_X_' + reg] - yields[reg]['IA_X'].yieldDictFull['tt']['QCD_IA_X_' + reg] - yields[reg]['IA_X'].yieldDictFull['z']['QCD_IA_X_' + reg]
            QCD_IXA[reg] =  yields[reg]['IXA'].yieldDictFull['dblind']['QCD_IXA_' + reg] -   yields[reg]['IXA'].yieldDictFull['w']['QCD_IXA_' + reg] -   yields[reg]['IXA'].yieldDictFull['tt']['QCD_IXA_' + reg] -   yields[reg]['IXA'].yieldDictFull['z']['QCD_IXA_' + reg]
          
            QCDexp1[reg] = (QCD_IX_A[reg] * QCD_XA_I[reg] * QCD_IA_X[reg])/(QCD_IXA[reg] * QCD_IXA[reg])

            QCDexp2[reg] = (yields[reg]['IX_A'].yieldDictFull['qcd']['QCD_IX_A_' + reg] * yields[reg]['XA_I'].yieldDictFull['qcd']['QCD_XA_I_' + reg] * \
            ...
            print makeLine()
            print "EWK-sub-data: nIX_A = ".replace("X", Xs[ABCD]), QCD_IA_X[reg], " | MC: nIX_A = ".replace("X", Xs[ABCD]), yields[reg]['IX_A'].yieldDictFull['qcd']['QCD_IX_A_' + reg],
            print "EWK-sub-data: nXA_I = ".replace("X", Xs[ABCD]), QCD_IX_A[reg], " | MC: nXA_I = ".replace("X", Xs[ABCD]), yields[reg]['XA_I'].yieldDictFull['qcd']['QCD_XA_I_' + reg],
            print "EWK-sub-data: nIA_X = ".replace("X", Xs[ABCD]), QCD_XA_I[reg], " | MC: nIA_X = ".replace("X", Xs[ABCD]), yields[reg]['IA_X'].yieldDictFull['qcd']['QCD_IA_X_' + reg],
            print "EWK-sub-data: nIXA = ".replace("X", Xs[ABCD]),  QCD_IXA[reg],  " | MC: nIXA = ".replace("X", Xs[ABCD]),  yields[reg]['IXA'].yieldDictFull['qcd']['QCD_IXA_' + reg]
            print "QCD Estimation in ", reg, ": ", QCDexp1[reg], " (", QCDexp2, ")"
            print "QCD MC yield in ", reg, ": ", yields[reg]['SR'].yieldDictFull['qcd']['QCD_SR_' + reg]
            print makeLine()
            
            with open("%s/QCDyields%s.txt"%(savedir,suffix), "a") as outfile:
               outfile.write(reg + "     " +\
               str(QCD_IX_A[reg].round(2)) + "             " +\
               str(QCD_XA_I[reg].round(2)) + "             " +\
               str(QCD_IA_X[reg].round(2)) + "             " +\
               str(QCD_IXA[reg].round(2)) + "             " +\
               str(QCDexp1[reg].round(2)) + "             " +\
               str(yields[reg]['SR'].yieldDictFull['qcd']['QCD_SR_' + reg].round(2)) + "             ")
               if yields[reg]['SR'].yieldDictFull['qcd']['QCD_SR_' + reg].val:
                  outfile.write(str((QCDexp1[reg]/yields[reg]['SR'].yieldDictFull['qcd']['QCD_SR_' + reg]).round(2)) + "\n")
               else:
                  outfile.write("\n")
 
         elif ABCD == "2" or ABCD == "3": #2D ABCD
            QCD_IX_A[reg] = yields[reg]['IX_A'].yieldDictFull['dblind']['QCD_IX_A_' + reg] - yields[reg]['IX_A'].yieldDictFull['w']['QCD_IX_A_' + reg] - yields[reg]['IX_A'].yieldDictFull['tt']['QCD_IX_A_' + reg] - yields[reg]['IX_A'].yie
            QCD_A_IX[reg] = yields[reg]['IA_X'].yieldDictFull['dblind']['QCD_IA_X_' + reg] - yields[reg]['IA_X'].yieldDictFull['w']['QCD_IA_X_' + reg] - yields[reg]['IA_X'].yieldDictFull['tt']['QCD_IA_X_' + reg] - yields[reg]['IA_X'].yie
            QCD_IXA[reg] =  yields[reg]['IXA'].yieldDictFull['dblind']['QCD_IXA_' + reg] -   yields[reg]['IXA'].yieldDictFull['w']['QCD_IXA_' + reg] -   yields[reg]['IXA'].yieldDictFull['tt']['QCD_IXA_' + reg] -   yields[reg]['IXA'].yiel
            ...
            print makeLine()
            print "EWK-sub-data: nIX_A = ".replace("X", Xs[ABCD]), QCD_IX_A[reg], " | QCD: nIX_A = ".replace("X", Xs[ABCD]), yields[reg]['IX_A'].yieldDictFull['qcd']['QCD_IX_A_' + reg],
            print "EWK-sub-data: nA_IX = ".replace("X", Xs[ABCD]), QCD_A_IX[reg], " | QCD: nA_IX = ".replace("X", Xs[ABCD]), yields[reg]['A_IX'].yieldDictFull['qcd']['QCD_A_IX_' + reg],
            print "EWK-sub-data: nIXA = ".replace("X", Xs[ABCD]),  QCD_IXA[reg],  " | QCD: nIXA = ".replace("X", Xs[ABCD]),  yields[reg]['IXA'].yieldDictFull['qcd']['QCD_IXA_' + reg]
            print "QCD Estimation in ", reg, ": ", QCDexp1[reg], " (", QCDexp2, ")"
            print "QCD MC yield in ", reg, ": ", yields[reg]['SR'].yieldDictFull['qcd']['QCD_SR_' + reg]
            print makeLine()

            with open("%s/QCDyields_%s%s.txt"%(savedir,s,suffix), "a") as outfile:
               outfile.write(reg + "     " +\
               str(QCD_IX_A[reg].round(2)) + "             " +\
               str(QCD_A_IX[reg].round(2)) + "             " +\
               str(QCD_IXA[reg].round(2)) + "             " +\
               str(QCDexp1[reg].round(2)) + "             " +\
               str(yields[reg]['SR'].yieldDictFull[s]['QCD_SR_' + reg].round(2)) + "             ")
               if yields[reg]['SR'].yieldDictFull[s]['QCD_SR_' + reg].val:
                  outfile.write(str((QCDexp[reg]/yields[reg]['SR'].yieldDictFull[s]['QCD_SR_' + reg]).round(2)) + "\n")
               else:
                  outfile.write("\n")
...
if plot:
   if save: #web address: http://www.hephy.at/user/mzarucki/plots/electronID
      ...
      QCD[plotReg]['I_A'] = CutClass("QCD_I_A_SR1", [
         SRs['anti-I'][plotReg],
         ["A", dPhiCut], #applied
         ["anti-I", sumSel1(combineCuts(eleSel, antiHybIsoCut))], #inverted
         ], baseCut = presel)
      
      QCD[plotReg]['IA'] = CutClass("QCD_IA_SR1", [
         SRs['anti-I'][plotReg],
         ["anti-A", antidPhiCut], #inverted
         ["anti-I", sumSel1(combineCuts(eleSel, antiHybIsoCut))], #inverted
         ], baseCut = presel)
      ...
      if ABCD != "3":
         QCD[plotReg]['A_I'] = CutClass("QCD_A_I_SR1", [
            SRs['I'][plotReg],
            ["anti-A", antidPhiCut], #inverted
   ...
   for sel in plotRegions:
      ...
      else: plots2[sel] = drawPlots(samples, plotsDict[sel], QCD[plotReg][sel], samplesList, plotList = plotsList[sel], plotMin = 0.1, normalize = False, save=False)
