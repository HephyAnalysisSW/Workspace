...
def QCDplots(collection = "LepGood", samples = samples, logy = 0, save = save, savedir = savedir):
   ...
   #elePt_D = "Max$( " + collection + "_pt*(" + eleSel + "&&" + antiDxyCut + "))"
   ...
   #hybIso_D = "Max$(( " + collection + "_relIso03*min( " + collection + "_pt, 25))*(" + eleSel + "&&" + antiDxyCut + "))"
...
savedir = "/afs/hephy.at/user/m/mzarucki/www/plots/QCD/ABCD1/plots/ABCD/combined"
if not os.path.exists(savedir): os.makedirs(savedir)

selections = ['SR', 'ID_A', 'DA_I', 'IA_D', 'IDA']
