...
if save: #web address: http://www.hephy.at/user/mzarucki/plots/electronID
   ...
   savedir = "/afs/hephy.at/user/m/mzarucki/www/plots/%s/leptonSFs/factored/"%tag
...
plotVars = {'pt':{}, 'eta':{}}
plotVars['pt']['baseString'] = varSel(pt, baseString)
plotVars['eta']['baseString'] = varSel(eta, baseString)
plotVars['pt']['selString'] = varSel(pt, selString)
plotVars['eta']['selString'] = varSel(eta, selString)
...
if not varBins:
   ...
   if standardBins:
      bins = {'pt': array('d', range(10,50,10) + range(50,200+150,150))}
      normFactor = {'pt': "(({var} < 50) + ({var} >= 50 && {var} < 200)*0.0666)".format(var = pt)}
   else: 
      bins = {'pt': array('d', [0, 5, 12, 20, 30, 200])} 
      normFactor = {'pt': "(({var} < 5) + ({var} >= 5 && {var} < 12)*0.714 + ({var} >= 12 && {var} < 20)*0.625 + ({var} >= 20 && {var} < 30)*0.5 + ({var} >= 30 && {var} < 200)*0.0294)".format(var = pt)}
   ...
   if lep == 'mu':
      ...
      normFactor['eta'] = "(({var} < 0.9)*0.111 + ({var} >= 0.9 && {var} < 1.2)*0.333 + ({var} >= 1.2 && {var} < 2.1)*0.111 + ({var} >= 2.1 && {var} < 2.4)*0.333)".format(var = eta)
   elif lep == 'el':
      bins['eta'] = array('d', [0, 1.4442, 1.556, 2.5])
      normFactor['eta'] = "(({var} < 1.4442)*0.0692 + ({var} >= 1.4442 && {var} < 1.556)*0.894 + ({var} >= 1.556 && {var} < 2.5)*0.106)".format(var = eta)

if not normalise: normFactor = {'pt': "1", 'eta': "1"}
...
for sim in ['FullSim', 'FastSim']:
   ...
   #Efficiency
   if not varBins: 
      hists[sim]['pt']['den'] = makeHist(samples[sample].tree, plotVars['pt']['baseString'], "weight*(" + combineCutsList(baseSelList) + ")", bins['pt'][0], bins['pt'][1], bins['pt'][2])
      hists[sim]['pt']['den'].GetYaxis().SetTitle("Events")
   else: 
      hists[sim]['pt']['den'] = makeHistVarBins(samples[sample].tree, plotVars['pt']['baseString'], normFactor['pt']+ "*weight*(" + combineCutsList(baseSelList) + ")", bins['pt'])
   ...
   if not varBins: hists[sim]['pt']['num'] = makeHist(samples[sample].tree, plotVars['pt']['baseString'], "weight*(" + combineCutsList(selList) + ")", bins['pt'][0], bins['pt'][1], bins['pt'][2])
   else: hists[sim]['pt']['num'] = makeHistVarBins(samples[sample].tree, plotVars['pt']['baseString'], normFactor['pt'] + "*weight*(" + combineCutsList(selList) + ")", bins['pt'])
   ...
   #Efficiency
   if not varBins: 
      hists[sim]['eta']['den'] = makeHist(samples[sample].tree, plotVars['eta']['baseString'], "weight*(" + combineCutsList(baseSelList) + ")", bins['eta'][0], bins['eta'][1], bins['eta'][2])
      hists[sim]['eta']['den'].GetYaxis().SetTitle("Events")
   else: 
      hists[sim]['eta']['den'] = makeHistVarBins(samples[sample].tree, plotVars['eta']['baseString'], normFactor['eta']+ "*weight*(" + combineCutsList(baseSelList) + ")", bins['eta'])
   ...
   if not varBins: hists[sim]['eta']['num'] = makeHist(samples[sample].tree, plotVars['eta']['baseString'], "weight*(" + combineCutsList(selList) + ")", bins['eta'][0], bins['eta'][1], bins['eta'][2])
   else: hists[sim]['eta']['num'] = makeHistVarBins(samples[sample].tree, plotVars['eta']['baseString'], normFactor['eta'] + "*weight*(" + combineCutsList(selList) + ")", bins['eta'])
   ...
   if not varBins: hists[sim]['2D']['den'] = make2DHist(samples[sample].tree, plotVars['pt']['baseString'], plotVars['eta']['baseString'], "weight*(" + combineCutsList(baseSelList) + ")", bins['pt'][0], bins['pt'][1], bins['pt'][2], bins['eta'][0], bins['eta'][1], bins['eta'][2])
   else: hists[sim]['2D']['den'] = make2DHistVarBins(samples[sample].tree, plotVars['pt']['baseString'], plotVars['eta']['baseString'], normFactor['pt'] + "*" + normFactor['eta'] + "*weight*(" + combineCutsList(baseSelList) + ")", bins['pt'], bins['eta'])
   ...
   #2D Histograms (wrt. pT)
   if not varBins: hists[sim]['2D']['num'] = make2DHist(samples[sample].tree, plotVars['pt']['baseString'], plotVars['eta']['baseString'], "weight*(" + combineCutsList(selList) + ")", bins['pt'][0], bins['pt'][1], bins['pt'][2], bins['eta'][0], bins['eta'][1], bins['eta'][2])
   else: hists[sim]['2D']['num'] = make2DHistVarBins(samples[sample].tree, plotVars['pt']['baseString'], plotVars['eta']['baseString'], normFactor['pt'] + "*" + normFactor['eta'] + "*weight*(" + combineCutsList(selList) + ")", bins['pt'], bins['eta'])
...
ratios['Full-Fast']['pt'].SetTitle("%ss: Factorised HI & IP FullSim-FastSim SFs for TTJets Sample"%lepton)
...
ratios['Full-Fast']['eta'].SetTitle("%ss: Factorised HI & IP FullSim-FastSim SFs for TTJets Sample"%lepton)
...
if save:
   c7.SaveAs("%s/TTJets_Full-FastSimSFs_eta%s.png"%(savedir, suffix))
   c7.SaveAs("%s/pdf/TTJets_Full-FastSimSFs_eta%s.pdf"%(savedir, suffix))
   c7.SaveAs("%s/root/TTJets_Full-FastSimSFs_eta%s.root"%(savedir, suffix))
...
for sim in ['FullSim', 'FastSim']:
   ...
   #if varBins and not standardBins:  
