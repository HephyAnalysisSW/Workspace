...
dir = "/afs/hephy.at/user/m/mzarucki/www/plots/filter/8TeV/HT/genMETonly/filter_%s_%s/"%(str(cuts["genMET"]),str(cuts["genHT"]))
...
for i in range(1,19):
   files.append(ROOT.TFile(dir + massRanges[0] + "/redMassScanMETonly_%s_%s_"%(str(cuts["genMET"]),str(cuts["genHT"])) + str(i) + ".root", "read"))

for i in range(19,33):
   files.append(ROOT.TFile(dir + massRanges[1] + "/redMassScanMETonly_%s_%s_"%(str(cuts["genMET"]),str(cuts["genHT"])) + str(i) + ".root", "read"))

for i in range(33,44):
   files.append(ROOT.TFile(dir + massRanges[2] + "/redMassScanMETonly_%s_%s_"%(str(cuts["genMET"]),str(cuts["genHT"])) + str(i) + ".root", "read"))

for i in range(44,57):
   files.append(ROOT.TFile(dir + massRanges[3] + "/redMassScanMETonly_%s_%s_"%(str(cuts["genMET"]),str(cuts["genHT"])) + str(i) + ".root", "read"))

for i in range(57,62):
   files.append(ROOT.TFile(dir + massRanges[4] + "/redMassScanMETonly_%s_%s_"%(str(cuts["genMET"]),str(cuts["genHT"])) + str(i) + ".root", "read"))
...
c1.cd(1)
total.SetTitle("Generated MET Filter: Total Number of Events per Mass Point for genMET > " + str(cuts['genMET']) + " GeV")
...
c1.cd(2)
passed.SetTitle("Generated MET Filter: Passed Generator Filter Events per Mass Point for genMET > " + str(cuts['genMET']) + " GeV")
...
c2.cd(1)
redFactor = ROOT.TH2F("redFactor", "Generated MET Filter: Reduction Factor per Mass Point Map for genMET > " + str(cuts['genMET']) + " GeV", 80, 50, 450, 18, 0, 90)
...
c2.cd(2)
redFactorPlot = ROOT.TH1F("redFactorPlot", "Generated MET Filter: Reduction Factor vs. Mass Point Plot for genMET > " + str(cuts['genMET']) + " GeV", 105, 0, 105)
...
#Write to file
savedir = "/afs/hephy.at/user/m/mzarucki/www/plots/filter/8TeV/HT/genMETonly/filter_%s_%s/"%(str(cuts["genMET"]),str(cuts["genHT"])) #web directory http://www.hephy.at/user/mzarucki/plots/filter/

if not os.path.exists(savedir):
   os.makedirs(savedir)

c1.SaveAs(savedir + "redMassScanTotal1_gMET_%s"%(str(cuts['genMET'])) + ".root")
c1.SaveAs(savedir + "redMassScanTotal1_gMET_%s"%(str(cuts['genMET'])) + ".png")
c1.SaveAs(savedir + "redMassScanTotal1_gMET_%s"%(str(cuts['genMET'])) + ".pdf")

c2.SaveAs(savedir + "redMassScanTotal2_gMET_%s"%(str(cuts['genMET'])) + ".root")
c2.SaveAs(savedir + "redMassScanTotal2_gMET_%s"%(str(cuts['genMET'])) + ".png")
c2.SaveAs(savedir + "redMassScanTotal2_gMET_%s"%(str(cuts['genMET'])) + ".pdf")

c3.SaveAs(savedir + "redMassScanTotalMap_gMET_%s"%(str(cuts['genMET'])) + ".root")
c3.SaveAs(savedir + "redMassScanTotalMap_gMET_%s"%(str(cuts['genMET'])) + ".root")
c3.SaveAs(savedir + "redMassScanTotalMap_gMET_%s"%(str(cuts['genMET'])) + ".png")

c4.SaveAs(savedir + "redMassScanTotalPlot_gMET_%s"%(str(cuts['genMET'])) + ".pdf")
c4.SaveAs(savedir + "redMassScanTotalPlot_gMET_%s"%(str(cuts['genMET'])) + ".png")
c4.SaveAs(savedir + "redMassScanTotalPlot_gMET_%s"%(str(cuts['genMET'])) + ".pdf")
