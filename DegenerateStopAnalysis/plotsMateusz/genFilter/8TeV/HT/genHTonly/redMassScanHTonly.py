...
for iev,event in enumerate(events):
   ...
   if genHT > cuts['genHT']: #both gCuts
...
#Write to file
savedir = "/afs/hephy.at/user/m/mzarucki/www/plots/filter/8TeV/HT/genHTonly/filter_%s_%s/"%(str(cuts["genMET"]),str(cuts["genHT"])) + str(options.stopMasses) #web directory http://www.hephy.at/user/mzarucki/plots/filter/

if not os.path.exists(savedir):
   os.makedirs(savedir)

c1.SaveAs(savedir + "/redMassScanHTonly_%s_%s_"%(str(cuts['genMET']), str(cuts['genHT'])) + str(options.jobNumber) + ".root")
c1.SaveAs(savedir + "/redMassScanHTonly_%s_%s_"%(str(cuts['genMET']), str(cuts['genHT'])) + str(options.jobNumber) + ".png")
c1.SaveAs(savedir + "/redMassScanHTonly_%s_%s_"%(str(cuts['genMET']), str(cuts['genHT'])) + str(options.jobNumber) + ".pdf")
