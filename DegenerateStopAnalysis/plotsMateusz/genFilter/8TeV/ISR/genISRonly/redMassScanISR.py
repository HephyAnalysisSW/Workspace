...
#dir =  "" #"//dpm/oeaw.ac.at/home/cms/store/user/schoef/pat_240614/T2DegenerateStop_2J_mStop-100to150-2/" #"/afs/hephy.at/work/w/wadam/CMSSW_5_3_20/src/T2DegenerateStop_2J_mStop-100to150-4/"
...
parser.add_option("--masses", dest="stopMasses",  help="stop mass range", type="str", default=None)
...
#   hists[massPoint][0] = ROOT.TH1F(massPoint + "_gMET", massPoint + "_gMET", nbins, min, max)
#   hists[massPoint][1] = ROOT.TH1F(massPoint + "_gMET2", massPoint + "_gMET2", nbins, min, max)
#   #hists[massPoint][2] = ROOT.TH1F(massPoint + "_gISR", massPoint + "_gISR", nbins, min, max) #red factor same when both cuts applied
#   #hists[massPoint][3] = ROOT.TH1F(massPoint + "_gISR2", massPoint + "_gISR2", nbins, min, max)
...
for iev,event in enumerate(events):
   ...
   if genISRpt > cuts['gISR']: #both gCuts
...
#Write to file
savedir = "/afs/hephy.at/user/m/mzarucki/www/plots/filter/8TeV/ISR/genISRonly/filter_%s_%s/"%(str(cuts["gMET"]),str(cuts["gISR"])) + str(options.stopMasses) #web directory http://www.hephy.at/user/mzarucki/plots/filter/

if not os.path.exists(savedir):
   os.makedirs(savedir)

c1.SaveAs(savedir + "/redMassScan_gISR_%s_"%(str(cuts['gISR'])) + str(options.jobNumber) + ".root")
c1.SaveAs(savedir + "/redMassScan_gISR_%s_"%(str(cuts['gISR'])) + str(options.jobNumber) + ".png")
c1.SaveAs(savedir + "/redMassScan_gISR_%s_"%(str(cuts['gISR'])) + str(options.jobNumber) + ".pdf")
