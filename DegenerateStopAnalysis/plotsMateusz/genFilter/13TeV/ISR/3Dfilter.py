...
'gMET' : 145, #generated quantity cuts
'gISR' : 0,
'gEta' : 5
})
...
#Save to Web
savedir = "/afs/hephy.at/user/m/mzarucki/www/plots/filter/13TeV/ROI/finalCuts/"

if not os.path.exists(savedir):
      os.makedirs(savedir)

c1.SaveAs(savedir + "%s_%s/3Dfilter_%s_%s.root"%(str(cuts['gMET']), str(cuts['gISR']), str(cuts['gMET']), str(cuts['gISR'])))
c1.SaveAs(savedir + "%s_%s/3Dfilter_%s_%s.png"%(str(cuts['gMET']), str(cuts['gISR']), str(cuts['gMET']), str(cuts['gISR'])))
c1.SaveAs(savedir + "%s_%s/3Dfilter_%s_%s.pdf"%(str(cuts['gMET']), str(cuts['gISR']), str(cuts['gMET']), str(cuts['gISR'])))
