...
h2.Draw("COLZ") #CONT1-5 #plots the graph with axes and points
...
#h1.SetMinimum(0)
#h1.SetMaximum(2E7)
h3.Draw("COLZ") #CONT1-5 #plots the graph with axes and points
...
#Save to Web
savedir = "/afs/hephy.at/user/m/mzarucki/www/plots/filter/ROI/finalCuts/"

if not os.path.exists(savedir):
      os.makedirs(savedir)

c1.SaveAs(savedir + "%s_%s/2Dfilter_%s_%s.root"%(str(cuts['gMET']), str(cuts['gISR']), str(cuts['gMET']), str(cuts['gISR'])))
c1.SaveAs(savedir + "%s_%s/2Dfilter_%s_%s.png"%(str(cuts['gMET']), str(cuts['gISR']), str(cuts['gMET']), str(cuts['gISR'])))
c1.SaveAs(savedir + "%s_%s/2Dfilter_%s_%s.pdf"%(str(cuts['gMET']), str(cuts['gISR']), str(cuts['gMET']), str(cuts['gISR'])))
