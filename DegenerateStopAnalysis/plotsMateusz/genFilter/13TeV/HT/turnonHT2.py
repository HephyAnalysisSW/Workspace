#turnonHT2.py - Imported by filter.py. Plots turn-on curves for MET and HT, as a function of genMET and genHT cuts.

def main(genMETcut, genHTcut): #main function to be imported by filter.py.
   print "\nImporting and executing turnonHT2.py script..."
...
************************************************************************************************************************************************************************\n"
   ...
   #
   #c1.cd(1)
   #h1 = drawhist(T2DegSample, var, presel1, nbins, min, max) #HT preselection cut
   #h1.SetName("MET")
   ...
   #box1 = makeBox()
   #box1.AddText("Cuts:")
   #box1.AddText("#bf{H_{T} cut: }" + str(cuts['HT']) + " GeV")
   #box1.AddText("#bf{H_{T} Jets p_{T} cut: }" + str(cuts['HTjetPt']) + " GeV")
   #box1.AddText("#bf{H_{T} Jets #eta cut: }" + str(cuts['HTjetEta']))
   ...
   c2.cd(1)
   h5 = drawhist(T2DegSample, var, gensel1, nbins, min, max)
   h5.SetName("MET")
   h5.SetTitle("Generated MET & H_{T} Filter Effect on Reconstructed MET (genMET Cut in Total)")
   h5.Draw() 
   ...
   l2 = makeLegend()
   l2.AddEntry("MET", "MET (genMET cut)", "F")
   
   ROOT.gPad.SetLogy()
   
   h6 = drawhist(T2DegSample, var, gensel1 + "&&" + gensel2) # + HT generator cut
   h6.SetName("MET2")
   h6.Draw("same")
   h6.SetFillColor(0)
   ...
   metTurnon2.SetTitle("MET Turnon Plot (Both Generator Cuts | genMET Cut in Total) ; Missing Transverse Energy #slash{E}_{T} / GeV ; Counts")
   ...
   #
   #l4 = makeLegend()
   #l4.AddEntry("HT", "H_{T} (MET preselection cut)", "F")
   #
   #ROOT.gPad.SetLogy()
   #ROOT.gPad.Update()
   #
   #alignStats(h13)
   #
   #h14 = drawhist(T2DegSample, var, presel2 + "&&" + gensel2) # + HT generator cut
   #h14.SetName("HT1")
   #h14.Draw("same")
   #h14.SetFillColor(0)
   ...
   #h14.SetLineWidth(4)
   #
   #l4.AddEntry("HT1", "H_{T} (generator cut)", "F")
   #l4.Draw()
   #
   ##Efficiency and Reduction Factor Calculation 
   ...
   #h16 = drawhist(T2DegSample, var, gensel2) #genHT cut
   #
   ...
   #htTurnon1.SetTitle("H_{T} Turnon Plot (Single Generator Cut) ; H_{T} / GeV ; Counts")
   ...
   c5.cd(1)
   h17 = drawhist(T2DegSample, var, gensel2)
   h17.SetName("HT")
   h17.SetTitle("Generated H_{T} & MET Filter Effect on Reconstructed H_{T} (genHT Cut in Total)")
   ...
   l5 = makeLegend()
   l5.AddEntry("HT", "H_{T} (genHT cut)", "F")
   
   ROOT.gPad.SetLogy()
   
   h18 = drawhist(T2DegSample, var, gensel2 + "&&" + gensel1) # + MET generator cut
   h18.SetName("HT2")
   h18.Draw("same")
   h18.SetFillColor(0)
   ...
   l5.AddEntry("HT2", "H_{T} (both generator cuts)", "F")
   l5.Draw()
   ...
   box9.AddText("#bf{Gen. H_{T} cut: }" + str(cuts['genHT']) + " GeV")
   box9.AddText("#bf{Gen. H_{T} Jets p_{T} cut: }" + str(cuts['genHTjetPt']) + " GeV")
   box9.AddText("#bf{Gen. H_{T} Jets #eta cut: }" + str(cuts['genHTjetEta']))
   box9.AddText("Filter:")
   ...
   htTurnon2.SetTitle("H_{T} Turnon Plot (Both Generator Cuts | genHT Cut in Total) ; H_{T} / GeV ; Counts")
   ...
   #Write to file
   savedir = "/afs/hephy.at/user/m/mzarucki/www/plots/filter/13TeV/HT/genCutInTotal/filter_%s_%s"%(str(cuts["genMET"]),str(cuts["genHT"])) #web address: http://www.hephy.at/user/mzarucki/plots/filter/
   ...
   "HT2       ", "    ", eff5, "     ", "         ", red5, "      ", recoEff5_bin, "                ", recoEff5_fit, "\n", \
   makeLine(), "\n", \
   "Turn-on fit results:", "\n\n", \
   ...
   "HT2", "  ", fit5[0], "  ", fit5[1], "  ", fit5[2], "  ", fit5[3], "  ", fit5[4], "  ", fit5[5], "  ", fit5[6], "  ", fit5[7], "  ", fit5[8], "\n", \
   makeLine(), "\n", \
   ...
   "HT2        ", fit5[9], fit5[10], fit5[11], fit5[12], fit5[13], fit5[14], fit5[15], fit5[16]
   outfile1.close()
   ...
   outfile2.write(\
   "genMET Cut" + "   " + "genHT Cut" + "MET 2 Red. Factor" + "    " + "MET 2 Reco Eff." + "    " + "HT 2 Red. Factor" + "    " + "HT 2 Reco Eff." + "\n" +\
   "  " + str(cuts["genMET"]) + "         " + str(cuts["genHT"]) + "         " + str(red2) + "      " + str(recoEff2_bin) + "      " + "      " + str(red5) + "      " + str(recoEff5_bin)\
   )
   outfile2.close()
   
   #Save to Web
   #c1.SaveAs(savedir + "/MET/MET1_%s_%s.root"%( str(cuts['genMET']), str(cuts['genHT'])))
   c2.SaveAs(savedir + "/MET/MET2_%s_%s.root"%(str(cuts['genMET']), str(cuts['genHT'])))
   ...
   c5.SaveAs(savedir + "/HT/HT2_%s_%s.root"%(str(cuts['genMET']), str(cuts['genHT'])))
   ...
   c2.SaveAs(savedir + "/MET/MET2_%s_%s.png"%(str(cuts['genMET']), str(cuts['genHT'])))
   ...
   c5.SaveAs(savedir + "/HT/HT2_%s_%s.png"%(str(cuts['genMET']), str(cuts['genHT'])))
   ...
   c2.SaveAs(savedir + "/MET/MET2_%s_%s.pdf"%(str(cuts['genMET']), str(cuts['genHT'])))
   ...
   c5.SaveAs(savedir + "/HT/HT2_%s_%s.pdf"%(str(cuts['genMET']), str(cuts['genHT'])))
