#redEffMapMHT.py script - Produces 2D map of reduction factors and efficiencies at reco, for varying genHT and genMHT cuts.

print "\nExecuting redEffMapMHT.py script..."
...
dir = "/afs/hephy.at/user/m/mzarucki/www/plots/filter/13TeV/MHT/"
...
#Gets all file dirs with filter results
...
for filename in files:
   ...
   for line in infile:
         ...
         #print line
         buffer = line.split()
         cutsHT.append(buffer[0]) #gHT cut
         cutsMHT.append(buffer[1]) #gISR cut
         redFactor1.append(buffer[2]) #HT 1
         effReco1.append(buffer[3]) #HT 1
         redFactor2.append(buffer[4]) #HT 2
         effReco2.append(buffer[5]) #HT 2
...
gr1 = ROOT.TGraph2D(len(effReco2), np.array(cutsHT, 'float64'), np.array(cutsMHT, 'float64'), np.array(redFactor2, 'float64')) #graph object with error bars using arrays of data
gr1.SetTitle("Total Reduction Factor")
gr1.GetHistogram()
gr1.GetXaxis().SetTitle("genHT Cut / GeV")
gr1.GetYaxis().SetTitle("genMHT Cut / GeV")
...
gr1.GetXaxis().SetRangeUser(100, 190)
gr1.GetYaxis().SetRangeUser(0, 120)
...
#totalReduction
gr2 = ROOT.TGraph2D(len(effReco2), np.array(cutsHT, 'float64'), np.array(cutsMHT, 'float64'), np.array(effReco2, 'float64')) #graph object with error bars using arrays of data
gr2.SetTitle("Generated HT Filter Efficiency at Reco Cut Value")
 
gr2.GetHistogram()
gr2.GetXaxis().SetTitle("genHT Cut / GeV")
gr2.GetYaxis().SetTitle("genMHT Cut / GeV")
...
gr2.GetXaxis().SetRangeUser(100, 190)
...
gr3 = ROOT.TGraph2D(len(effReco2), np.array(cutsHT, 'float64'), np.array(cutsMHT, 'float64'), np.array(effReco4, 'float64')) #graph object with error bars using arrays of data
gr3.SetTitle("Generated MHT Filter Efficiency at Reco Cut Value")
 
gr3.GetHistogram()
gr3.GetXaxis().SetTitle("genHT Cut / GeV")
gr3.GetYaxis().SetTitle("genMHT Cut / GeV")
...
gr3.GetXaxis().SetRangeUser(100, 190)
