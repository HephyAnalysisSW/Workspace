...
#  print structString
  ...
  for bin in sample["bins"]:
    if small: sample["filenames"][bin] = sample["filenames"][bin][:1]
    for thisfile in sample["filenames"][bin]:
      ...
      if ntot>0:
        ...
        print "Reading: ", sample["name"], bin,"file",file, "with",number_events,"Events using cut", commoncf
        if small:
