...
from helpers import getObjFromFile, passPUJetID, getISRweight, getHardestMuon, calcMT
...
#    if met>200 and met<300 and nbtags==0:
    lepton = getHardestMuon(c)
    if lepton and lepton['pdg']*charge<0 and abs(lepton['eta'])<1.5 and lepton['pt']<20:
      return True
...
#  additionalCutFunc = "type1phiMet>200&&type1phiMet<300&&nbtags==0&&softIsolatedMuPdg>0&&abs(softIsolatedMuEta)<1.5"
  additionalCutFunc = cutFunc(-1) 
  addData = True
  addSignals = True
...
prefix="empty_"
if presel == "cr23Sel":
  commoncf="isrJetPt>110&&type1phiMet>200&&type1phiMet<300&&nbtags==0&&isrJetBTBVetoPassed&&nHardElectrons+nHardTaus==0&&njet60<=2"

prefix= "HardLepton_"+presel+"_"+chmode+"_"
if preprefix!="":
...
if doAnalysisVars:
  mT_stack  = getStack(":XXX;m_{T} (GeV);Number of Events / 10 GeV",[21,0,210], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = lambda c: calcMT(getHardestMuon(c), {'pt':getVarValue(c, 'type1phiMet'), 'phi':getVarValue(c, 'type1phiMetphi')}))
  mT_stack[0].addOverFlowBin = "upper"
  allStacks.append(mT_stack)

#  isrJetEta_stack = getStack(":isrJetEta;#eta of ISR jet;Number of Events",[20,-5,5], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  isrJetEta_stack[0].addOverFlowBin = "upper"
#  allStacks.append(isrJetEta_stack)
#
#  hardIsolatedMuPt_stack = getStack(":XXX;p_{T} of hard isolated muon;Number of Events / 10 GeV",[25,0,250], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = lambda c: getHardestMuon(c)['pt'])
#  hardIsolatedMuPt_stack[0].addOverFlowBin = "upper"
#  allStacks.append(hardIsolatedMuPt_stack)
#
#  met_stack = getStack(":type1phiMet;#slash{E}_{T} (GeV);Number of Events / 50 GeV",[18,150,1050], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  met_stack[0].addOverFlowBin = "upper"
#  allStacks.append(met_stack)
#
#  metNoMETCut_stack = getStack(":type1phiMet;#slash{E}_{T} (GeV);Number of Events / 50 GeV",[18,150,1050], commoncf.replace('&&type1phiMet>200','').replace('&&type1phiMet<300','').replace('&&type1phiMet>300',''), signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  metNoMETCut_stack[0].addOverFlowBin = "upper"
#  allStacks.append(metNoMETCut_stack)
#
#  njet_stack = getStack(":njet;n_{jet};Number of Events",[10,0,10], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  njet_stack[0].addOverFlowBin = "upper"
#  allStacks.append(njet_stack)
#
#  njet60_stack = getStack(":njet60;n_{jet};Number of Events",[10,0,10], commoncf.replace('&&njet60<=2',''), signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  njet60_stack[0].addOverFlowBin = "upper"
#  allStacks.append(njet60_stack)
#  
#  nbtags_stack = getStack(":nbtags;n_{b-tags};Number of Events",[10,0,10], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  nbtags_stack[0].addOverFlowBin = "upper"
#  allStacks.append(nbtags_stack)
#
#  ht_stack                          = getStack(":ht;H_{T} (GeV);Number of Events / 50 GeV",[31,0,1550 ], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  ht_stack[0].addOverFlowBin = "upper"
#  allStacks.append(ht_stack)
#
#  isrJetPt_stack = getStack(":isrJetPt;p_{T} of ISR jet;Number of Events / 20 GeV",[50,0,1000], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  isrJetPt_stack[0].addOverFlowBin = "upper"
#  allStacks.append(isrJetPt_stack)
#
#  hardIsolatedMuEta_stack = getStack(":XXX;#eta of hard isolated muon;Number of Events",[20,-4,4], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = lambda c: getHardestMuon(c)['eta'])
#  hardIsolatedMuEta_stack[0].addOverFlowBin = "both"
#  allStacks.append(hardIsolatedMuEta_stack)
#
#  hardIsolatedMuCharge_stack = getStack(":XXX;charge of hard isolated muon;Number of Events",[3,-1,2], commoncf, signals, varfunc = lambda c: getHardestMuon(c)['pdg']/(-13), addData = addData, additionalCutFunc = additionalCutFunc)
#  hardIsolatedMuCharge_stack[0].addOverFlowBin = "both"
#  allStacks.append(hardIsolatedMuCharge_stack)
...
#    return t
#
#  htThrustMetSide_stack = getStack(":xxx;htThrustMetSide;Number of Events",[50,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = htThrustMetSideFunc)
#  htThrustMetSide_stack[0].addOverFlowBin = "upper"
#  allStacks.append(htThrustMetSide_stack)
#
##  closestMuJetMass_stack = getStack(":xxx;closestMuJetMass;Number of Events",[50,0,500], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = closestMuJetMass)
##  closestMuJetMass_stack[0].addOverFlowBin = "upper"
##  allStacks.append(closestMuJetMass_stack)
##
##  closestMuJetDeltaR_stack = getStack(":xxx;closestMuJetDeltaR;Number of Events",[50,0,7], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = closestMuJetDeltaR)
##  closestMuJetDeltaR_stack[0].addOverFlowBin = "upper"
##  allStacks.append(closestMuJetDeltaR_stack)
##
##  closestMuJetDeltaR_zoomed_stack = getStack(":xxx;closestMuJetDeltaR;Number of Events",[50,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = closestMuJetDeltaR)
##  closestMuJetDeltaR_zoomed_stack[0].addOverFlowBin = "upper"
##  allStacks.append(closestMuJetDeltaR_zoomed_stack)
#
#  cosPhiMetJet_stack = getStack(":xxx;cos(#phi(#slash{E}_{T}, ISR-jet));Number of Events",[20,-1,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = lambda c: cos(getVarValue(c, 'isrJetPhi') - getVarValue(c, 'softIsolatedMuPhi')))
#  cosPhiMetJet_stack[0].addOverFlowBin = "both"
#  allStacks.append(cosPhiMetJet_stack)
#
#  FWMT1_stack = getStack(":xxx;FMWT1 (jets,lep,MET);Number of Events",[20,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = lambda c:foxWolframMoments(c)['FWMT1'])
#  FWMT1_stack[0].addOverFlowBin = "upper"
#  allStacks.append(FWMT1_stack)
#
#  FWMT2_stack = getStack(":xxx;FMWT2 (jets,lep,MET);Number of Events",[20,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = lambda c:foxWolframMoments(c)['FWMT2'])
#  FWMT2_stack[0].addOverFlowBin = "upper"
#  allStacks.append(FWMT2_stack)
#
#  FWMT3_stack = getStack(":xxx;FMWT3 (jets,lep,MET);Number of Events",[20,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = lambda c:foxWolframMoments(c)['FWMT3'])
#  FWMT3_stack[0].addOverFlowBin = "upper"
#  allStacks.append(FWMT3_stack)
#
#  FWMT4_stack = getStack(":xxx;FMWT4 (jets,lep,MET);Number of Events",[20,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = lambda c:foxWolframMoments(c)['FWMT4'])
#  FWMT4_stack[0].addOverFlowBin = "upper"
#  allStacks.append(FWMT4_stack)
#
#
#  c2D_stack = getStack(":xxx;c2D (jets,lep,MET);Number of Events",[20,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = lambda c:circularity2D(c)['c2D'])
#  c2D_stack[0].addOverFlowBin = "upper"
#  allStacks.append(c2D_stack)
#
#  linC2D_stack = getStack(":xxx;linC2D (jets,lep,MET);Number of Events",[20,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = lambda c:circularity2D(c)['linC2D'])
#  linC2D_stack[0].addOverFlowBin = "upper"
#  allStacks.append(linC2D_stack)
#
#  thrust_stack = getStack(":xxx;thrust;Number of Events",[20,0.6,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = lambda c: thrust(c)['thrust'])
#  thrust_stack[0].addOverFlowBin = "upper"
#  allStacks.append(thrust_stack)
#
#  htThrustLepSide_stack = getStack(":xxx;htThrustLepSide;Number of Events",[50,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc, varfunc = lambda c: thrust(c)['htThrustLepSide'])
#  htThrustLepSide_stack[0].addOverFlowBin = "upper"
#  allStacks.append(htThrustLepSide_stack)
#
#  sTlep_stack  = getStack(":XXX;S_{T, lep.} (GeV);Number of Events / 50 GeV",[21,0,1050], commoncf, signals, lambda c: c.GetLeaf('softIsolatedMuPt').GetValue() + c.GetLeaf('type1phiMet').GetValue() , addData = addData, additionalCutFunc = additionalCutFunc)
#  sTlep_stack[0].addOverFlowBin = "upper"
#  allStacks.append(sTlep_stack)
#
#  cosDeltaPhiLepMET_stack  = getStack(":xxx;cos(#Delta #phi(l, #slash{E}_{T}));Number of Events",[30,-1.1,1.1], commoncf, signals, lambda c: cos(c.GetLeaf('softIsolatedMuPhi').GetValue() - c.GetLeaf('type1phiMetphi').GetValue()), addData = addData, additionalCutFunc = additionalCutFunc)
#  allStacks.append(cosDeltaPhiLepMET_stack)
#
#  cosDeltaPhiLepW_stack  = getStack(":xxx;cos(#Delta #phi(l, #slash{E}_{T}));Number of Events",[30,-1.1,1.1], commoncf, signals, cosDeltaPhiLepW, addData = addData, additionalCutFunc = additionalCutFunc)
#  allStacks.append(cosDeltaPhiLepW_stack)
#
##  ptw_stack = getStack(":sqrt((metpx + leptonPt*cos(lepton_phi))**2 + (metpy + leptonPt*sin(lepton_phi))**2);(l+#slash{E})_{T}; Number of Events / 25 GeV",[21,0,525], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
...
#
#  softIsolatedMuPhi_stack = getStack(":softIsolatedMuPhi;#phi of soft isolated muon;Number of Events",[20,-4,4], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  softIsolatedMuPhi_stack[0].addOverFlowBin = "both"
#  allStacks.append(softIsolatedMuPhi_stack)
#
#  softIsolatedMuRelIso_stack = getStack(":softIsolatedMuRelIso;I_{rel.} of soft isolated muon;Number of Events",[24,0,1.2], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  softIsolatedMuRelIso_stack[0].addOverFlowBin = "both"
#  allStacks.append(softIsolatedMuRelIso_stack)
#
#  softIsolatedMuAbsIso_stack = getStack(":XXX;I_{abs.} of soft isolated muon;Number of Events",[30,0,12], commoncf, signals, lambda c: c.GetLeaf('softIsolatedMuRelIso').GetValue()*c.GetLeaf('softIsolatedMuPt').GetValue(), addData = addData, additionalCutFunc = additionalCutFunc)
#  softIsolatedMuAbsIso_stack[0].addOverFlowBin = "both"
#  allStacks.append(softIsolatedMuAbsIso_stack)
#
#  softIsolatedMuDxy_stack = getStack(":softIsolatedMuDxy;d_{xy} of soft isolated muon;Number of Events",[40,0,.08], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
...
#if doISRJetVars:
#
#  isrJetPhi_stack = getStack(":isrJetPhi;#phi of ISR jet;Number of Events",[20,-pi,pi], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  isrJetPhi_stack[0].addOverFlowBin = "upper"
#  allStacks.append(isrJetPhi_stack)
#
#  isrJetBtag_stack = getStack(":isrJetBtag;CSV b-tag of ISR jet;Number of Events",[20,-1,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  isrJetBtag_stack[0].addOverFlowBin = "upper"
#  allStacks.append(isrJetBtag_stack)
#
#  isrJetChef_stack = getStack(":isrJetChef;Chef of ISR jet;Number of Events",[20,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  isrJetChef_stack[0].addOverFlowBin = "upper"
#  allStacks.append(isrJetChef_stack)
#
#  isrJetNhef_stack = getStack(":isrJetNhef;Nhef of ISR jet;Number of Events",[20,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  isrJetNhef_stack[0].addOverFlowBin = "upper"
#  allStacks.append(isrJetNhef_stack)
#
#  isrJetCeef_stack = getStack(":isrJetCeef;Ceef of ISR jet;Number of Events",[20,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  isrJetCeef_stack[0].addOverFlowBin = "upper"
#  allStacks.append(isrJetCeef_stack)
#
#  isrJetNeef_stack = getStack(":isrJetNeef;Neef of ISR jet;Number of Events",[20,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  isrJetNeef_stack[0].addOverFlowBin = "upper"
#  allStacks.append(isrJetNeef_stack)
#
#  isrJetHFhef_stack = getStack(":isrJetHFhef;HFhef of ISR jet;Number of Events",[20,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  isrJetHFhef_stack[0].addOverFlowBin = "upper"
#  allStacks.append(isrJetHFhef_stack)
#
#  isrJetHFeef_stack = getStack(":isrJetHFeef;HFeef of ISR jet;Number of Events",[20,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  isrJetHFeef_stack[0].addOverFlowBin = "upper"
#  allStacks.append(isrJetHFeef_stack)
#
#  isrJetMuef_stack = getStack(":isrJetMuef;Muef of ISR jet;Number of Events",[20,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  isrJetMuef_stack[0].addOverFlowBin = "upper"
#  allStacks.append(isrJetMuef_stack)
#
#  isrJetElef_stack = getStack(":isrJetElef;Elef of ISR jet;Number of Events",[20,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  isrJetElef_stack[0].addOverFlowBin = "upper"
#  allStacks.append(isrJetElef_stack)
#
#  isrJetPhef_stack = getStack(":isrJetPhef;Phef of ISR jet;Number of Events",[20,0,1], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  isrJetPhef_stack[0].addOverFlowBin = "upper"
#  allStacks.append(isrJetPhef_stack)
#
#if doOtherVars:
#  type1phiMetphi_stack = getStack(":type1phiMetphi;#phi(#slash{E}_{T});Number of Events",[40,-pi,pi], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  type1phiMetphi_stack[0].addOverFlowBin = "upper"
#  allStacks.append(type1phiMetphi_stack)
#
#  ngoodVertices_stack = getStack(":ngoodVertices;Number of Vertices;Number of Events",[61,0,61], commoncf, signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  ngoodVertices_stack[0].addOverFlowBin = "upper"
#  allStacks.append(ngoodVertices_stack)
#
#  njetWOjetCut_stack = getStack(":njet;n_{jet};Number of Events",[10,0,10], commoncf.replace('&&njet60<=2',''), signals, addData = addData, additionalCutFunc = additionalCutFunc)
#  njetWOjetCut_stack[0].addOverFlowBin = "upper"
#  allStacks.append(njetWOjetCut_stack)
#  
#
#  isrJetFull53XPUJetIDTight_stack = getStack(":isrJetFull53XPUJetIDTight;isrJetFull53XPUJetIDTight;Number of Events",[2,0,2], commoncf, signals,lambda c: passPUJetID(int(c.GetLeaf('isrJetFull53XPUJetIDFlag').GetValue()),'Tight'), addData = addData, additionalCutFunc = additionalCutFunc, onlyW=True)
#  allStacks.append(isrJetFull53XPUJetIDTight_stack)
#  isrJetFull53XPUJetIDMedium_stack = getStack(":isrJetFull53XPUJetIDMedium;isrJetFull53XPUJetIDMedium;Number of Events",[2,0,2], commoncf, signals,lambda c: passPUJetID(int(c.GetLeaf('isrJetFull53XPUJetIDFlag').GetValue()),'Medium'), addData = addData, additionalCutFunc = additionalCutFunc, onlyW=True)
#  allStacks.append(isrJetFull53XPUJetIDMedium_stack)
#  isrJetFull53XPUJetIDLoose_stack = getStack(":isrJetFull53XPUJetIDLoose;isrJetFull53XPUJetIDLoose;Number of Events",[2,0,2], commoncf, signals,lambda c: passPUJetID(int(c.GetLeaf('isrJetFull53XPUJetIDFlag').GetValue()),'Loose'), addData = addData, additionalCutFunc = additionalCutFunc, onlyW=True)
#  allStacks.append(isrJetFull53XPUJetIDLoose_stack)
#  
#  isrJetMET53XPUJetIDTight_stack = getStack(":isrJetMET53XPUJetIDTight;isrJetMET53XPUJetIDTight;Number of Events",[2,0,2], commoncf, signals,lambda c: passPUJetID(int(c.GetLeaf('isrJetMET53XPUJetIDFlag').GetValue()),'Tight'), addData = addData, additionalCutFunc = additionalCutFunc, onlyW=True)
#  allStacks.append(isrJetMET53XPUJetIDTight_stack)
#  isrJetMET53XPUJetIDMedium_stack = getStack(":isrJetMET53XPUJetIDMedium;isrJetMET53XPUJetIDMedium;Number of Events",[2,0,2], commoncf, signals,lambda c: passPUJetID(int(c.GetLeaf('isrJetMET53XPUJetIDFlag').GetValue()),'Medium'), addData = addData, additionalCutFunc = additionalCutFunc, onlyW=True)
#  allStacks.append(isrJetMET53XPUJetIDMedium_stack)
#  isrJetMET53XPUJetIDLoose_stack = getStack(":isrJetMET53XPUJetIDLoose;isrJetMET53XPUJetIDLoose;Number of Events",[2,0,2], commoncf, signals,lambda c: passPUJetID(int(c.GetLeaf('isrJetMET53XPUJetIDFlag').GetValue()),'Loose'), addData = addData, additionalCutFunc = additionalCutFunc, onlyW=True)
#  allStacks.append(isrJetMET53XPUJetIDLoose_stack)
#  
#  isrJetCutBasedPUJetIDTight_stack = getStack(":isrJetCutBasedPUJetIDTight;isrJetCutBasedPUJetIDTight;Number of Events",[2,0,2], commoncf, signals,lambda c: passPUJetID(int(c.GetLeaf('isrJetCutBasedPUJetIDFlag').GetValue()),'Tight'), addData = addData, additionalCutFunc = additionalCutFunc, onlyW=True)
#  allStacks.append(isrJetCutBasedPUJetIDTight_stack)
#  isrJetCutBasedPUJetIDMedium_stack = getStack(":isrJetCutBasedPUJetIDMedium;isrJetCutBasedPUJetIDMedium;Number of Events",[2,0,2], commoncf, signals,lambda c: passPUJetID(int(c.GetLeaf('isrJetCutBasedPUJetIDFlag').GetValue()),'Medium'), addData = addData, additionalCutFunc = additionalCutFunc, onlyW=True)
#  allStacks.append(isrJetCutBasedPUJetIDMedium_stack)
#  isrJetCutBasedPUJetIDLoose_stack = getStack(":isrJetCutBasedPUJetIDLoose;isrJetCutBasedPUJetIDLoose;Number of Events",[2,0,2], commoncf, signals,lambda c: passPUJetID(int(c.GetLeaf('isrJetCutBasedPUJetIDFlag').GetValue()),'Loose'), addData = addData, additionalCutFunc = additionalCutFunc, onlyW=True)
...
#  drawNMStacks(1,1,[hardIsolatedMuPt_stack],            subdir+prefix+"hardIsolatedMuPt", False)
...
#  hardIsolatedMuEta_stack[0].maximum = 6*10**3 *hardIsolatedMuEta_stack[0].data_histo.GetMaximum()
#  drawNMStacks(1,1,[hardIsolatedMuEta_stack],            subdir+prefix+"hardIsolatedMuEta", False)
#  hardIsolatedMuCharge_stack[0].maximum = 6*10**3 *hardIsolatedMuCharge_stack[0].data_histo.GetMaximum()
#  drawNMStacks(1,1,[hardIsolatedMuCharge_stack],            subdir+prefix+"hardIsolatedMuCharge", False)
#if doAllDiscriminatingVars:
