...
parser.add_option("--cut", dest="cut", default='candId==6&&candEta>0', type="string", action="store", help="conditions:Which conditions")
...
c.Draw('atan2(Sum$(('+options.cut+')*candPt*sin(candPhi)),Sum$(('+options.cut+')*candPt*cos(candPhi)))>>hStart(30,-pi,pi)','','goff') #goff - no graphics is generated
hStart=ROOT.gDirectory.Get('hStart') #ohne Korrektur
hStart.SetLineColor(ROOT.kRed)
history = [hStart] #schreibt hStart als erstes Listenelement in history
...
#  f = 'atan2(Sum$(('+candRequ+')*candPt*sin(atan2(sin(candPhi)+'+str(aprime)+'*abs(sinh(candEta))*sin('+str(phiHF)+'), cos(candPhi)+'+str(aprime)+'*abs(sinh(candEta))*cos('+str(phiHF)+')))),Sum$(('+candRequ+')*candPt*cos(atan2(sin(candPhi)+'+str(aprime)+'*abs(sinh(candEta))*sin('+str(phiHF)+'), cos(candPhi)+'+str(aprime)+'*abs(sinh(candEta))*cos('+str(phiHF)+')))))'
  ...
  if verbose:  print f #prints current function
...
#  if verbose:print 'Chi2',fRes.Chi2(),fRes.Ndf(), 'Target:',res, 'at xHF/phiHF',x 
  ...
  history.append(h.Clone('iter_'+str(iter))) #clone: kopiert h mit namen "iter_i", append: kopiertes h wird in historyliste gesetzt
  history[0].Draw()
  history[-1].Draw('same')

#  for hist in history[-1:]: #warum wir das so gemacht? Was es tut: immer das letzte listenelement zeichnen
 #   hist.Draw('same') 
  lines=[]
  ...
  c1.Modified()
  c1.Update()
  c1.Print('/afs/hephy.at/user/'+userName[0]+'/'+userName+'/www/pngHF/metPhiHF_'+postFix+'.gif+10')

  iter+=1
  if verbose:print 'Target:',res, 'at dx/dy',x, 'sample', options.sample # documentation if verbose is activated
  del h
  return res 
...
c1.Print('/afs/hephy.at/user/'+userName[0]+'/'+userName+'/www/pngHF/metPhiHF_'+postFix+'.gif++') #infinite animation dedicated in the last image written
...
latex.SetTextAlign(11); # align right
for line in lines:
  latex.SetTextSize(0.04)  #noted twice????
