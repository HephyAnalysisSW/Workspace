...
nTrueGenSignalHomeGrown = getObjFromFile("/data/schoef/monoJetStuff/nTrueGenVertices_HomeGrownSignals.root", "hFullSim").Clone("nTrueGenSignal")
nTrueGenSignalHomeGrown.Scale(1./nTrueGenSignalHomeGrown.Integral())
...
def getPUReweightingUncertainty(mcScenario = "S10", dataFile = "/data/schoef/results2012/PU/MyDataPileupHistogram_Run2012ABCD_60max_true_pixelcorr_Sys0.root"):
  if mcScenario=="S10":
    mcHist = S10
  if mcScenario=="S7":
    mcHist = S7
  if mcScenario == "homeGrownSignal":
    mcHist = nTrueGenSignalHomeGrown 
  if mcHist.Integral()!=1:
    mcHist.Scale(1./mcHist.Integral())
  
  reweightingHisto = getObjFromFile(dataFile, "pileup").Clone("rwHist")
  reweightingHisto.Scale(1./reweightingHisto.Integral())
  reweightingHisto.Divide(mcHist)
  return reweightingHisto 
